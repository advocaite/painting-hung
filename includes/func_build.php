...
{
	$hs=array(1=>200,2=>100,3=>150);
	if(isset($_SESSION['nation_id']))
	{	
		return $level*$hs[$_SESSION['nation_id']];
	}
	else
	{
		global $user;		
		return $level*$hs[$user['nation_id']];
	}
}
...
function SCn_eco_oth($level)
{
	...
	$value=array("0"=>800,"1"=>1210,"2"=>2020,"3"=>3230,"4"=>4840,"5"=>6850,"6"=>9260,"7"=>12070,"8"=>15280,"9"=>18890,"10"=>22900,"11"=>27310,"12"=>32120,"13"=>37330,"14"=>42940,"15"=>48950,"16"=>55360,"17"=>62170,"18"=>69380,"19"=>76990,"20"=>85000,"21"=>93410,"22"=>102528,"23"=>115800,"24"=>130208,"25"=>145800,"26"=>162624,"27"=>180728,"28"=>200160,"29"=>220968,"30"=>243200);
	return $value[$level];
}
/*-------------------------------------------------------------------------------------------------------------------------------
* @Author: duc hien
...
function KTn_res($level)
{
	...
	$value=array("0"=>2,"1"=>3,"2"=>4,"3"=>6,"4"=>9,"5"=>13,"6"=>19,"7"=>27,"8"=>39,"9"=>56,"10"=>79,"11"=>111,"12"=>156,"13"=>217,"14"=>300,"15"=>412,"16"=>562,"17"=>762,"18"=>1026,"19"=>1373,"20"=>1825,"21"=>2409,"22"=>3158,"23"=>4112,"24"=>5317,"25"=>6827);
	return $value[$level];
...
function destroyByCata($id,$type_id,$level,$village)
{
	...
	if($level-1 <=0)
	{
		...
		else
		{
			...
			if($type_id==36)
			{
				$sql="UPDATE wg_buildings SET name='City_Wall',img='',type_id=36,level=0,product_hour=0,cp=0 WHERE id=".$id;
			}
			else
			{
...
function DeleteStatus($id,$village,$type_status,$object_id)
{
	...
	if($type_status==1)
	{
		...
		$sql="UPDATE wg_buildings SET name='$name',img='',level=0,type_id=$typeId,product_hour=0 
...
function Creat_Allies($userid)
{
	...
	{
		if(strip_tags($_REQUEST['ally1'])=="" || strip_tags($_REQUEST['ally1'])==" ")
		{
			$check++;
			$error1=$lang['error1'];
		}
		if(strip_tags($_REQUEST['ally2'])==""  || strip_tags($_REQUEST['ally1'])==" ")
		{
			$check++;
			$error2=$lang['error2'];
		}
		...
		else
		{
			...
			else
			{
				// 2. so sanh tai nguyen can nang cap va tai nguyen hien co
					...
					{	//kiem tra doanh trai, hoc vien co du dieu kien nang cap khong?
						if($type_id == 28 && $level_1 >= 3 && get_level_building(31,$village) < 5)
						{
							$dieu_kien=$lang['dieu_kien0'];
						}
						elseif($type_id == 31 && $level_1 >= 4 && get_level_building(12,$village) < 10)
						{
							$dieu_kien=$lang['dieu_kien01'];
						}
						else
						{
							//cap nhat tinh trang chien tranh cho lang co dinh thu >=10
							include_once('function_troop.php');
							if((get_level_building(12,$village) >= 10) && (check_iswar(GetPlayerID($village)) == 0))
							{
								//cap nhat tinh trang chien tranh
								$sql = "update wg_users set iswar = 1 where id = ".GetPlayerID($village);
								$db->setQuery($sql);
								$db->query();
							}
							$char='village1.php';					
							...
							{
								if($level_1 >= 9 && get_level_building(12,$village) < 10)
								{
									$dieu_kien=$lang['dieu_kien01'];
								}
								else
...
{
	global $db,$game_config;
	$sql="SELECT id,`index`,type_id,level FROM wg_buildings WHERE vila_id=".$villages_id." ORDER BY `index` ASC";
	$db->setQuery($sql);
	$wg_buildings=NULL;
	...
	if($wg_buildings)
	{
		foreach($wg_buildings as $value)
		{
			...
			if($value->index >18 && $value->level==0)
			{
				$product_hour=0;
			}		
			if($value->level >0)
...
function returnKrsForUser($userid)
{
    global $db;
    $sql="SELECT id FROM wg_villages WHERE user_id=".$userid." AND kind_id < 7";
    $db->setQuery($sql);
    ...
    if($wg_villages)
    {
        ...
        {
            $arr_hsRS = array();
            $arr_hsRS = returnKrsForVillage($userid,$result->id,$time_check);
            $string = substr($arr_hsRS['char'],- strlen($arr_hsRS['char'])+1);
            $sql="UPDATE wg_villages SET $string WHERE id = ".$result->id;
            $db->setQuery($sql);
            $db->query();
        }        
    }
...
// lay level hien tai cua mot cong trinh
function get_level_building($type_building, $village_id)
{
	global $db;
	$sql="Select level from wg_buildings where type_id = ".$type_building." and vila_id = ".$village_id;
	$db->setQuery($sql);
	$db->loadObject($level_building);
	return $level_building->level;
}
?>
