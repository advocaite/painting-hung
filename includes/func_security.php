...
function globalError($content)
{
	...
	session_destroy();
	$sql="INSERT INTO wg_error (time,content) VALUES ('".date("Y-m-d H:i:s")."','".$db->getEscaped($content)."')";
	$db->setQuery($sql);
	$db->query();
