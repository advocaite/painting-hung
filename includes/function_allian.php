...
function listVillageAndAllyMember($userid, $village)
{
    ...
    $sql   = "SELECT kind_id,id,x,y,name FROM wg_villages WHERE user_id=" . $userid
        . " AND kind_id < 7 ORDER BY workers DESC";
    $db->setQuery($sql);
        ...
        foreach ($elements as $ptu)
        {
            if ($ptu->kind_id <= 6)
            {
                ...
                if (isset($_SESSION['url']))
                {
                    ...
                    if ($ptu->id == $village)
                    {
                        $parse['link'] = '<span class="c2">&#8226;</span>&nbsp;<a href="' . $char
                            . '" class="active_vl">' . $name_village . '</a>';
                    }
                }
                else
                {
                    $parse['link']
                        = '<span >&#8226;</span>&nbsp;<a href="village1.php?vid=' . $ptu->id . '">' . $name_village
                        . '</a>';

                    if ($ptu->id == $village)
                    {
                        $parse['link'] = '<span class="c2">&#8226;</span>&nbsp;<a href="village1.php?vid=' . $ptu->id
                            . '" class="active_vl">' . $name_village . '</a>';
                    }
                }
    ...
    {
        $parse['Alliances'] = '<a href="allianz.php"><span style="color:#CC3333">' . $lang['Alliances'] . '</span></a>';
        $parse['Hinh_alle'] = '<img src="images/un/a/lien_minh.PNG" border="0" height="18" width="18"/>';
        $allyId             = $user['alliance_id'];
        // truy van qua bang wg_ally_members lay cac userid member
        $sql                =
            "SELECT tb1.user_id,tb2.username FROM wg_ally_members as tb1,wg_users as tb2 WHERE tb1.ally_id = " . $allyId
            . " AND tb1.right_ = 1  AND tb2.id=tb1.user_id ORDER BY tb2.population DESC";
        $db->setQuery($sql);
        $elements = $db->loadObjectList();
    ...
    if ($lang['event_server'] != '')
    {
        $parse['event_server'] = $lang['event_server'];
        $parse['retaining']    = '';
        $time_now              = strtotime(date("Y-m-d H:i:s"));

        if ($time_now < $lang['begin'])
        {
            $parse['retaining'] = '<span id="EventServer_1">' . ReturnTime($lang['begin'] - $time_now) . '</span>';
        }

        if ($time_now >= $lang['begin'] && $time_now <= $lang['end'])
        {
            header("Location:logout.php");
            exit();
        }

        $parse['event_server'] = parsetemplate(gettemplate('event_server'), $parse);
    }
...
function createAlly($userid, $level)
{
    ...
    if ($level - 1 < 3) // toa dai su co level < 3
    {
        ...
        if (Check_Allies_Invite($userid,
                                0) == 0 && Check_Allies_Invite($userid, 1) == 0 && Check_Allies_Exist($userid) == 0)
        {
        ...
        elseif (Check_Allies_Invite($userid, 1) == 0 && Check_Allies_Invite($userid, 0) > 0)
        {
            ...
            if (is_numeric($_GET['aid']) && is_numeric($_GET['accept']))
            {
                ...
                {
                    $sql = "UPDATE wg_ally_members SET right_=1,privilege = '00000001' WHERE id =" . $_GET['accept']
                        . " AND right_=0";
                    $db->setQuery($sql);
                    $db->query();

                    $sql = "UPDATE wg_users SET alliance_id=(SELECT ally_id FROM wg_ally_members WHERE user_id ="
                        . $userid . " AND right_=1) WHERE id=" . $userid . "";
                    $db->setQuery($sql);
                    $db->query();
    ...
    else
    {
        ...
        if ($level - 1 >= 5)
        {
            $parse['doi_do'] = "<a href=\"allianz.php?event=7\"><button>Doi do</button></a>";
        }

        // chua co loi moi gia nhap allie + chua tao allie cho rieng minh
        if (Check_Allies_Invite($userid,
                                0) == 0 && Check_Allies_Exist($userid) == 0 && Check_Allies_Invite($userid, 1) == 0)
        {
            // tao lien minh
        ...
        elseif (Check_Allies_Invite($userid, 1) == 0 && Check_Allies_Invite($userid, 0) > 0)
        {
            ...
            if (is_numeric($_GET['aid']) && is_numeric($_GET['accept']))
            {
                ...
                { //thong bao thanh vien trong lien minh da du        
                    // khung tao lien minh
                    $curent_product = Creat_Allies($userid);
        ...
        elseif (Check_Allies_Invite($userid, 1) > 0 || Check_Allies_Exist($userid) == 1)
        {
            ...
            {
                $sql
                    = "SELECT name,tag FROM wg_allies WHERE id=(SELECT ally_id FROM wg_ally_members WHERE user_id=$userid AND right_=1)";
            }
...
function Update_Glod_RS_Mission($village, $userid, $id)
{
    ...
    {
        $sql = "UPDATE wg_villages SET rs1=rs1+" . $wg_mission->rs1 . ",rs2=rs2+" . $wg_mission->rs2 . ",rs3=rs3+"
            . $wg_mission->rs3 . ",rs4=rs4+" . $wg_mission->rs4 . " WHERE id=$village AND kind_id < 7";
        $db->setQuery($sql);
        $db->query();
    }
...
function InsertReportQuest7($userid)
{
    ...
    $sql          =
        "INSERT INTO `wg_reports` (`user_id`, `title`, `time`, `report_text`, `status`, `type`) VALUES($userid,'"
        . $lang['report_title'] . "', '" . date('Y-m-d H:i:s') . "','" . $content . "',0,$type)";
    $db->setQuery($sql);
    ...
    $sql_bk
        = "INSERT INTO `wg_reports_bk` (`user_id`, `title`, `time`, `report_text`, `status`, `type`) VALUES($userid,'"
        . $lang['report_title'] . "', '" . date('Y-m-d H:i:s') . "','" . $content . "',0,$type)";
    $db->setQuery($sql_bk);
...
function imagesNation($villa_id)
{
    ...
    $img = array
    (
        1 => 'images/un/q/l1.jpg',
        2 => 'images/un/q/l2.jpg',
        3 => 'images/un/q/l3.jpg'
    );

    $img1 = array
    (
        1 => 'images/un/q/l1a.jpg',
        2 => 'images/un/q/l2a.jpg',
        3 => 'images/un/q/l3a.jpg'
    );
...
function maxMemberAllyHave($allyId)
{
    global $db;
    $maxMemberAlly = 0;
    $sql           = "SELECT id FROM wg_villages WHERE user_id =(SELECT user_id FROM wg_allies WHERE id = " . $allyId
        . ") AND kind_id < 7";
    $db->setQuery($sql);
    $$villages_id = NULL;
    $villages_id  = $db->loadObjectList();
    ...
    if ($villages_id)
    {
        foreach ($villages_id as $key => $v)
        {
            $sql = "SELECT product_hour FROM wg_buildings WHERE vila_id = " . $v->id . " AND type_id = 14";
            $db->setQuery($sql);
            $maxMember = null;
            $db->loadObject($maxMember);

            if ($maxMember->product_hour > $maxMemberAlly)
            {
                $maxMemberAlly = $maxMember->product_hour;
            }
        }
    }

    return $maxMemberAlly;
}
...
function getDiplomacyOwner($allyId1)
{
    ...
    if ($ownOffers)
    {
        foreach ($ownOffers as $ownOffer)
        {
            ...
            if ($type == 1)
                $typeName = $lang['Diplomacy type 11']; //"??i ??u";

            if ($type == 2)
                $typeName = $lang['Diplomacy type 21']; //"H?a h?o";

            if ($type == 3)
                $typeName = $lang['Diplomacy type 31']; //"??ng minh";
            ...
            $parse['relation_des_own'] = $typeName . " v?i " . $allyNameTo;
            $parse['status']           = "Ch?a ph?n h?i";
...
function getDiplomacyForeign($allyId2)
{
    ...
    if ($foreignOffers)
    {
        foreach ($foreignOffers as $foreignOffer)
        {
            ...
            if ($type == 2)
                $typeName = $lang['Diplomacy type 21']; //"H?u h?o";
            ...
            $parse['relation_des_foreign'] = $typeName . " v?i " . $allyName1;
...
function getInvitationList($allyId)
{
    ...
    WHERE tb.right_ = 0 AND tb.ally_id = " . $allyId
        . " ORDER BY tb.id DESC";
    $db->setQuery($sql);
