...
function attackOasisFree($attack, $status, &$villageAttack, &$oasis, &$heroAttack, &$attackTroopList, $plusAtt, $arrayTroop){
	...
	if($speedBack>0){
		...
		if($status->level >0)
		{
			$duration=intval($duration/$status->level);
		}
		if($attack->type == 11){
			...
			if($heroAttack->num==1 && $heroAttack->die_num==0){
				...
				$chckFd=checkFeudatory($villageAttack, $oasis, $arrInfo, $s);
				switch($chckFd){
					case 1:
					...
					case 2:
						...
						break;
					case 3:
						$arrInfo['thu_phuc']=$lang['bo_lac_ngoai_pham_vi'];
						break;
				}
			}
		}
		...
		//keo linh ve
		InsertStatusTroopAttackBack($attack->id, $attack->village_attack_id, date("Y-m-d H:i:s", strtotime($status->time_end)), date("Y-m-d H:i:s", strtotime($status->time_end)+$duration), $duration,$status->level);
	}else{
		//truong hop bai tran:
...
function attackOasisDepend($attack, $status, &$villageAttack, &$oasis, &$heroAttack, &$attackTroopList, $plusAtt, $arrayTroop){
	...
	if($speedBack>0){
		...
		if($attack->type == 11){
			if($heroAttack->num==1 && $heroAttack->die_num==0){
				switch(checkFeudatory($villageAttack, $oasis, $arrInfo, $s)){
					case 1:
						$arrInfo['thu_phuc']=$lang['da_thu_phuc_bo_lac'];
						break;
					case 2:
 ...
 * @return bool
 */
function checkFeudatory(&$villageAttack, &$oasis, &$arrInfo, &$s){	
	global $db;
	$result=0;
	...
	if($sumFeudation<$maxFeudation){
		//cap nhat ngay 10/06/09: khong chiem bo lac ngoai pham vi
		//kiem tra bo lac co nam trong pham vi cua lang danh khong
		if ($s <= 6)
		{
			...
			if($oasis->faith<35){
				...
				if($oasis->user_id){
					...
					$oasis->faith=100;
					$sql="UPDATE `wg_villages` SET `dateCreate_vila` = now() WHERE id = ".$oasis->id;
					$db->setQuery($sql);
					if(!$db->query()){
						globalError2("Loi cap nhap ngay tao cho oasis:".$sql);
					}
				}else{
					$oasis->faith=100;
		...
		{
			//thong bao bo lac nam ngoai pham vi
			$result=3;
		}			
	}
	return $result;
...
function insertOasis($oasis_id, $x, $y, $kind_id, $user_id, $village_id, $faith, $nation_id){
	global $db;
	$sql="INSERT INTO `wg_villages` (`id`, `x`, `y`, `kind_id`, `user_id`, `child_id`, `faith`, `nation_id`, `dateCreate_vila`) VALUES
($oasis_id, $x, $y, $kind_id, $user_id, $village_id, $faith, $nation_id, now())";
	$db->setQuery($sql);
	if(!$db->query()){
...
function attack($attack, $status){
	...
	if($defendTroopList && $attackTroopList){
		$kata=0;
		//Lay level lien minh cua lang bi danh
		include_once("function_troop.php");
		$aliLevel = GetLevelAli(GetUserAli_id($villageDefend->user_id));
		phase($attackTroopList, $heroAttack, $defendTroopList, $heroDefendList, $wallDefend, $kata, $attack->type, $plusAtt, $plusDef, $aliLevel);
	}
	...
	//cap nhat child_id cho lang danh
	if($chiemLang == 1)
	{
		include_once("function_troop.php");
		//Them so lang con cho lang thang tran:
		AddChildForVillage($villageAttack->id, $villageDefend->id);
		//cap nhat lai user_id trong bang wg_villages_map
		$sql = "update wg_villages_map set user_id = ".$villageAttack->user_id." where id = ".$villageDefend->id;
		$db->setQuery($sql);
		$db->query();
	}
...
function attackCatapult($attack, $status){
	...
	include_once("function_troop.php");
	$aliLevel = GetLevelAli(GetUserAli_id($villageDefend->user_id));
	phase($attackTroopList, $heroAttack, $defendTroopList, $heroDefendList, $wallDefend, $sumKata, $attack->type, $plusAtt, $plusDef, $aliLevel);
 ...
 * @return $attackTroops, $defendTroops hai mang 3 chieu luu luc luong cua 2 ben sau khi danh
 */
function phase(&$troopAttackList, &$heroAttack, &$troopDefendList, &$heroDefendList, $wallDefend=1, &$kata=0, $type=0, $plusAtt=0, $plusDef=0, $AliLevel=0){
...
//-------------------tinh tong phong th?----------------->	
	...
	//Tang % thu theo level cua Lien minh
	if($AliLevel > 0)
	{
		$sumDefend = $sumDefend + $sumDefend * $AliLevel/100;
	}
...
function writeAttackReport($arrAttacks, $heroAttack, $arrDefs, $heroDefendList, $bounty, $arrInfos, $timeAttack){	
	...
	//tru diem danh vong lang danh lang co dan thap qua lon
	if($userAttackId != $userDefId && ($arrInfos[0] != 3))
	{
		$unequal = (100 - (round(($objDefendVillage[0]->workers*100/$objAttackVillage[0]->workers),0)));
		if($unequal >50)
		{
			$arrInfos['culture'] = $lang['culture_info1'].($unequal - 50).$lang['culture_info2'];
			//tru diem danh vong cua nguoi danh
			
			//Start Sub culture for userAttack
			$uId =  $userAttackId;			
			...
			if($arrRes)
			{
				...
				foreach($arrRes as $r)
				{
					$cpNew = round($r->cp - ($r->cp * ($unequal - 50)/100),0);
					$sumCP = $sumCP + ($r->cp - $cpNew);
					$updateCultureUser = "UPDATE wg_villages SET cpupdate_time='$curr_date', cp= $cpNew WHERE id=$r->id";
					$db->setQuery($updateCultureUser);
					$db->Query();
				}
			}	
			//End sub curture for userAttack
			$arrInfos['culture'] .= ' (= '.$sumCP.' điểm.)'; 
		}
	}
	...
	if(!$bounty){
		...
		//cap nhat ngay 08/06/09: Khong tinh diem cuop boc 2 lang cung tai khoan
		if ($userAttackId != $userDefId)
		{
...
function executeUserAttackOasisPoint($arrAttacks, $arrDefs){	
	...
	if($oasisTroopKeepDie){				
		...
		//cap nhat ngay 08/06/09: Khong tinh diem tan cong khi danh bo lac
		//week ranking		
	...
	if(count($arrDfTrp)>0){
		foreach($arrDfTrp as $vlg_id=>$trpDf){
			if($vlg_id==$vlgDf->id){
				...
				$lstTrpVlg=GetListTroopVilla($village);
			}
			...
			$dfTables.=$dfTable;
			if($vlg_id != $vlgDf->id && $vlgDf->user_id!=$village->user_id){
