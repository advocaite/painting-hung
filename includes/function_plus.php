...
{
	global $db;
	$sql = "SELECT user_id,lumber,clay,iron,crop FROM wg_plus WHERE lumber >0 OR clay >0 OR iron >0 OR crop >0";	
	$db->setQuery($sql);	
	$wg_plus =NULL;
	$wg_plus= $db->loadObjectList();
	if($wg_plus)
	{
		foreach($wg_plus as $key=>$v)
		{
			$string=$char='';			
			if(strtotime($v->lumber) >0 && strtotime($v->lumber) < time() )
			{
				$string.="lumber='0000-00-00 00:00:00',";
				$char.='krs1=(krs1/1.25),';	
			}
			if(strtotime($v->clay) >0 && strtotime($v->clay) < time() )
			{
				$string.="clay='0000-00-00 00:00:00',";	
				$char.='krs2=(krs2/1.25),';						
			}
			if(strtotime($v->iron) >0 && strtotime($v->iron) < time() )
			{
				$string.="iron='0000-00-00 00:00:00',";
				$char.='krs3=(krs3/1.25),';			
			}
			if(strtotime($v->crop) >0 && strtotime($v->crop) < time() )
			{
				$string.="crop='0000-00-00 00:00:00',";	
				$char.='krs4=(krs4/1.25),';						
			}
			if($string !='')
			{
				$string=substr($string,0,-1);					
				$sql="UPDATE wg_plus SET ".$string." WHERE user_id = ".$v->user_id;
				$db->setQuery($sql);
				$db->query();
			...
			if($char !='')
			{
				...
				$sql="UPDATE wg_villages SET ".$char." WHERE user_id = ".$v->user_id." AND kind_id < 7";				
				$db->setQuery($sql);
				$db->query();
			}	
			unset($wg_plus[$key]);				
...
* @return: 
*/
function withdrawGold($gold,$userId,$asu_builling,$des,$logs,$currentValue=0)
{
	global $db,$user;	
	if(!$currentValue)
	{
	...
	{
		//$char=$logs.date("H:i d-m-y").',1,'.$des.','.$gold.';';
		//$field = "logs='".$char."',"
		$sql="UPDATE wg_plus SET gold = gold-".$gold." WHERE user_id=".$userId;
		$db->setQuery($sql);
		$db->query();
	...
	else
	{
		...
		{
			//$char=$logs.date("H:i d-m-y").',2,'.$des.','.($gold-$currentValue).';';
			//$field = "logs='".$char."',"
...
function getStatusForActiveNow($village_id)
{
	...
	$indexHeroMansion = getIndexOfHeroMansion($village_id);	
	$indexWonder = getIndexOfWonder($village_id);
	$sql = "SELECT id,object_id,type,time_end FROM wg_status 
...
function updateItemUser($userId,$name)
{
	global $db;	
	$sql="UPDATE wg_item_user SET quantity=quantity-1 WHERE user_id=".$userId." AND 	item_name='".$name."'";
	$db->setQuery($sql);
	return $db->query();
}
?>