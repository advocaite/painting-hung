...
function deleteTable($userId)
{
	...
	if($ally_id !=NULL)
	{
		...
		$sql = "SELECT user_id, wg_users.population FROM wg_ally_members LEFT JOIN wg_users ON wg_ally_members.user_id = wg_users.id WHERE ally_id = $ally_id AND right_ =1 ORDER BY `wg_users`.`population` DESC";
		$db->setQuery($sql);
