...
function executeConsumptionPeopleSoldiers($vId){
	...
	if($troopKeepSubtract){//Cap nhat lai suc an cua linh
        ...
        //ham nay giong doan lenh tren => tru troop keep cua thanh bi giet linh 2 lan => am troop keep
...
*/
function changeRSVillage($village_id, $rs1, $rs2, $rs3, $rs4, $asu=0, $item_amount=0, $item_name='')
{
	global $db;
	if($asu > 0)
	{
	...
	{	
        ...
        else
        {
            if($item_amount > 0)
            {
                $sql = "SELECT * FROM wg_item_user WHERE item_name = '$item_name' and user_id = ( SELECT user_id FROM wg_villages WHERE id =$village_id)";
                $db->setQuery($sql);
                $check=null;
                $db->loadObject($check);
                if($check)
                {
                    $query = "UPDATE `wg_item_user` SET `quantity` = `quantity` + $item_amount WHERE CONVERT( `item_name` USING utf8 ) = '$item_name' AND user_id = ( SELECT user_id FROM wg_villages WHERE id =$village_id)";
                    $db->setQuery($query);
                    $db->query();
                }
                else
                {
                    $sql_insert = "INSERT INTO `wg_item_user` (`user_id` ,`item_name` ,`quantity`) VALUES (( SELECT user_id FROM wg_villages WHERE id =$village_id), '$item_name', '$item_amount')";
                    $db->setQuery($sql_insert);echo $sql_insert;
                    $db->Query();
                }
            }
        }			
