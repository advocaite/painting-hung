...
function TransportRSStatus(){
	...
	$sql="SELECT
                ...
                wg_resource_sends.item_amount,
                wg_resource_sends.item_name,
	...
	if($transportList){
		...
		foreach($transportList as $transport){
            ...
            $parse['r6']=$transport->item_amount;
            $parse['r7']=$lang['Des_'.$transport->item_name];
            if($transport->item_name == '')
            {
                $parse['r7'] = 'Item';
            }
...
function executeResourceBuySale($status){
	...
	if($rsSend){
		...
		if(checkVillageID($rsSend->village_id_to)){
			//Cong rs cho lang nhan:
			changeRSVillage($rsSend->village_id_to, $rsSend->rs1, $rsSend->rs2, $rsSend->rs3, $rsSend->rs4, $rsSend->asu, $rsSend->item_amount, $rsSend->item_name);
		}else{
			require_once("function_troop.php");
			//tra rs ve lang gui:
			$oid = InsertSendRS($rsSend->village_id_from, $rsSend->village_id_to, $rsSend->rs1, $rsSend->rs2, $rsSend->rs3, $rsSend->rs4, $rsSend->asu,0, $rsSend->item_amount, $rsSend->item_name);
...
function reportTransport($status, $rsSend){
    ...
    $parse['item_amount']=$rsSend->item_amount;
    $parse['item_name']=$lang['Des_'.$rsSend->item_name];
