...
function ShowPalaceCP($building){
	...
	$parse['id'] = $_GET['id'];
	$parse['error'] = '';
	//gop diem cho lien minh
	if(count($_POST)<>0 && $_POST['input_diemDV'] >0)
	{
		$diemDV=intval($_POST['input_diemDV']);
		$AllCulture = allvillageCulturepoint();
		if($diemDV > $AllCulture)
		{
			$parse['error'] = "Số điểm lớn hơn số điểm danh vọng bạn có.";
		}
		else
		{
			...
			if(!$check)
			{
				$parse['error'] = "Bạn chưa có liên minh hay không thuộc liên minh nào.";
			}
			else
			{
				//cong diem danh vong cho lien minh
				$updateCultureAli = "UPDATE `wg_allies` SET `Culture` = `Culture` + $diemDV WHERE `wg_allies`.`id` = ".$check->ally_id;
				$db->setQuery($updateCultureAli);
				$db->Query();
				
				//tru diem danh vong cua nguoi gop
				$uId =  $user["id"];
				$sql="SELECT id, cp FROM wg_villages WHERE user_id=$uId and cp > 0";
				$db->setQuery($sql);
				...
				if($arrRes)
				{
					...
					foreach($arrRes as $r)
					{
						...
						if($r->id == $wg_village->id)
						{
							$wg_village->cp=$cpNew;
							$wg_village->cpupdate_time = $curr_date;
						}	
						$updateCultureUser = "UPDATE wg_villages SET cpupdate_time='$curr_date', cp= $cpNew WHERE id=$r->id";
						$db->setQuery($updateCultureUser);
						$db->Query();
				...
				//cap nhat tin tuc lien minh
				$content = '<a href=\"profile.php?uid=$uId\">'.$user['username'].'</a> đã góp <b>'.intval($_POST['input_diemDV']).'</b> điểm danh vọng cho liên minh';
				$sql ="INSERT INTO wg_ally_news(ally_id,content, time) VALUES(".$check->ally_id.",'".$content."','".date("Y-m-d H:i:s")."')";
				$db->setQuery($sql);
				$db->query();
				
				/*tinh level cho lien minh ((level +1) * (level + 2) * 50.000) diem tang 1 level.
				diem = trung binh cong cua diem danh vong va diem chien tran.
				level cua lien minh phu thuoc vao so diem*/
				
				$getAli = "SELECT `id` , `rank` , `Culture`, `point` FROM `wg_allies` WHERE `id` =(SELECT `ally_id` FROM `wg_ally_members` WHERE `user_id`=".$user['id']." and right_ = 1)";
				$db->setQuery($getAli);
				$db->loadObject($objGetAli);
				if($objGetAli)
				{	
					$n=0;
					$levelUp=$objGetAli->rank + 1;
					$pointLevelUp=$levelUp*($levelUp+1)*50000;
					$levelDown = $objGetAli->rank - 1;
					$pointLevelDown = $levelDown*($levelDown+1)*50000;
					$diemTB = ($objGetAli->Culture + $objGetAli->point)/2;
					if($diemTB >= $pointLevelUp)
					{
						while($diemTB > $pointLevelUp && $objGetAli->rank < 100)
						{
							$n++;
							$objGetAli->rank++;
							$levelUp=$objGetAli->rank+1;
					...
					{
						while($diemTB < $pointLevelDown && $objGetAli->rank > 0)
						{
							$n--;
							$objGetAli->rank--;
							$levelDown=$objGetAli->rank-1;
					...
					if($n != 0)
					{
						$sql="UPDATE `wg_allies` SET `rank` = `rank` + $n WHERE `wg_allies`.`id` =$objGetAli->id";
						$db->setQuery($sql);
						$db->Query();
					}
				}
...
function GetSumPresentTroop($troop_id){
	...
	$sql="SELECT 
			...
			WHERE
				...
				wg_attack.village_attack_id =  ".$user['villages_id']." AND
...
function GetSumTroopInVillage($troop_id){
	global $db, $wg_village, $user;
	$sql="SELECT num FROM wg_troop_villa WHERE troop_id = '$troop_id' AND village_id='$wg_village->id'";//".$user['villages_id']."";
	$db->setQuery($sql);
...
function rutQuanVienTro($building){
	global $db,$lang, $wg_village,$wg_config_plus,$user;
	...
	if($_GET['t']==5 && ($_GET['atid'])){
		...
		if($village_defend_id){
			...
			$duration=intval(($s/$speed)*3600);
			$parse['plus_speed']='';
			if($duration >3600)
			{
				$parse['duration1']=TimeToString($duration);
				$parse['duration2']=TimeToString(intval($duration/2));
				$parse['duration3']=TimeToString(intval($duration/3));
				$parse['duration4']=TimeToString(intval($duration/4));
				$parse['asu1']='';
				$parse['asu2']=$wg_config_plus['speed_troop_asu'];
				$parse['asu3']=$wg_config_plus['speed_troop_asu']*2;
				$parse['asu4']=$wg_config_plus['speed_troop_asu']*3;
				$parse['plus_speed']=parsetemplate(gettemplate('send_troop_body2a'), $parse);
			}
	...
	if(count($_POST)>0){
		...
		if($coQuan || $coHero){
			...
			if($duration >3600)
			{	
				$plus_speed=intval($_POST['plus_speed']);				
				if($plus_speed >=2 && $plus_speed <=4)
				{
					$asu_billing=get_gold_remote($user['username']);							
					$asu_need=$wg_config_plus['speed_troop_asu']*($plus_speed - 1);							
					if($asu_billing >= $asu_need)
					{
						$des=array(2=>18,3=>19,4=>20);								
						withdraw_gold_remote($user['username'],$asu_need,$des[$plus_speed]);
						InsertLogPlus($user['id'],$des[$plus_speed],$asu_need);	
						$duration=intval($duration/$plus_speed);							
					}							
				}
			}
...
function SendTroops($building){
	global $db, $lang, $wg_village,$user,$wg_config_plus;
	includeLang("send_troop");
	//Buoc 1:
	if(isset($_POST['vn'])){
	
		if($_POST['vn']){//echo 'AAAAAAAAAAAAA';
		...
		if($villageDefend->kind_id<7){
			//cap nhat tinh trang chien tranh cho lang co dinh thu >=10
			include_once('func_build.php');
			$userID_defend = GetPlayerID($villageDefend->id);
				if((get_level_building(12,$villageDefend->id)>=10)&& (check_iswar($userID_defend) == 0))
				{
					//cap nhat tinh trang chien tranh
					$sql = "update wg_users set iswar = 1 where id = ".$userID_defend;
					$db->setQuery($sql);
					$db->query();
				}
			//Kiem tra xem lang nay co thuoc dang can duoc bao ve hay khong:
			if(check_iswar($userID_defend)== 0)
			{
				return DisplaySendTroop1(6);
			}
			...
			if((get_level_building(12,$wg_village->id)>=10) && ($user['iswar'] == 0))
			{
				//cap nhat tinh trang chien tranh
				$sql = "update wg_users set iswar = 1 where id = ".$user['id'];
				$db->setQuery($sql);
				$db->query();
			}
	...
	if(is_numeric($_POST['vdid'])){
		if(is_numeric($_POST['st']) && ($_POST['st']==1 || $_POST['st']==2 || $_POST['st']==3 || $_POST['st']==4)){
			...
			if($villageDefend->kind_id<7){
				...
				$userID_defend = GetPlayerID($villageDefend->id);
				if((check_iswar($userID_defend)== 0) || ($user['iswar'] == 0))
				{
			...
			if($listTroop){
				if($villageDefend && $listTroop){
					...
					if($j>0 || ($_POST['t12'] && $hero)){
						...
						InsertAttackStatus($wg_village->id, $attack_id, date("Y:m:d H:i:s"), date("Y:m:d H:i:s", time()+ $duration),$duration,$plus_speed);
	...
	if($vla->user_id != $vld->user_id){
		...
		if(checkSMSAttack($vld->user_id, $vld->id)){
			...
			if($p){
				...
				//$asu = showGold($ud->id);
				$sql = "SELECT logs,gold FROM wg_plus WHERE user_id=".$ud->id;
				$db->setQuery($sql);
				$wg_plus=NULL;
				...
				if($asu>$asu_sms){
					//tru asu
					withdrawGold($asu_sms, $ud->id, $rmasu, 16,$wg_plus->logs);
...
function DisplaySendCatapult($villageDefend, $listTroop){
	global $db, $lang, $wg_village, $wg_buildings,$wg_config_plus;
	...
	//kiem tra do chenh lech dan so
	$parse['mess'] = '';
	if($wg_village->user_id != $villageDefend->user_id && ($_POST['st'] == 3 || $_POST['st'] == 4))
	{
		$unequal = (100 - (round(($villageDefend->workers*100/$wg_village->workers),0)));
		$mess = '<img src="images/warning1.gif"/> Độ chênh lệch dân số là: '.$unequal.'%.';
		if($unequal >50)
		{
			$parse['mess'] = $mess.' Bạn sẽ bị trừ '.($unequal - 50).'% điểm danh vọng.'; 	
		}
		else
		{
			$parse['mess'] = $mess;
...
{
	global $lang, $wg_village,$wg_config_plus;
	includelang("troop");
	$parse=$lang;
	...
	$duration=GetDuration($s, $speed);
	//Start: giam thoi gian vien tro giua cac thanh vien cung lien minh
	if($_POST['st']==2 && GetUserAli_id($wg_village->user_id)== GetUserAli_id($villageDefend->user_id))
	{
		$AliLevel = GetLevelAli(GetUserAli_id($wg_village->user_id));
		$duration = $duration - ($AliLevel * $duration / 100);
	}
	//End;
	$parse['duration']=TimeToString($duration);
	$parse['plus_speed']='';
	...
	$parse['village_defend_x']=$villageDefend->x;
	$parse['village_defend_y']=$wg_village->y;
...
function InsertAttackStatus($village_id, $object_id, $time_begin, $time_end, $cost_time,$level=0){
	global $db;
	$sql="INSERT INTO `wg_status` (`object_id`, `village_id`,`type`, `time_begin`, `time_end`, `cost_time`, `status`,`level`) VALUES ($object_id, $village_id, 7, '$time_begin', '$time_end', $cost_time, 0, $level)";
	$db->setQuery($sql);
	if($db->query()){
		return true;
	}else{
...
function InsertStatusTroopAttackBack($attack_id, $village_id, $time_begin, $time_end, $cost_time,$level=0){
	global $db;
	$sql="INSERT INTO wg_status (object_id, village_id, type, time_begin, time_end, cost_time, status,level) VALUES ($attack_id, $village_id, 10, '$time_begin', '$time_end', $cost_time, 0,$level)";
	$db->setQuery($sql);
	if(!$db->query()){
...
//kiem tra tai khoan nay da san sang chien tranh chua
function check_iswar($userID){
	global $db;
	$sql="SELECT iswar FROM wg_users WHERE id=$userID";
	$db->setQuery($sql);
	return $db->loadResult();
}
...
{
	global $db, $user, $lang, $wg_village,$wordFilters,$wg_config_plus, $wg_troops;
	includelang("hero");
	$parse=$lang;
	...
	switch($_GET['t']){
		case 1:
			...
			if($hero)
			{
				...
				{
					$asu_cung_menh=$wg_config_plus['cung_menh_asu'];					
					...
					if(is_numeric($_POST['cung']) && $_POST['cung']>0 && $_POST['cung']<=5 && $_POST['cung']!= $hero->cung_menh)
					{
						...
						if($asu_billing >= $asu_cung_menh)
						{
							...
							{	
								withdraw_gold_remote($user['username'],$asu_cung_menh,17);
								InsertLogPlus($user['id'],17,$asu_cung_menh);		
							}
		...
		case 3:
			...
			break;
		case 4://xep hang tuong
			require_once($ugamela_root_path . 'includes/wg_troops.php');
			includeLang('ranking');
			$parse=$lang;
			$parse['class_train']="";
			$parse['class_rank']="class=selected";
			$parse['error']=$parse['rank_hero']=$parse['value_name']=$parse['list']='';
			
			define('MAXROW',10);
			$rank=1;
			
			$sql="SELECT name,user_id,wg_users.username,troop_id,level,kinh_nghiem 
			FROM `wg_heros` AS tb INNER JOIN wg_users ON tb.user_id=wg_users.id WHERE `status` =1 ORDER BY `level` DESC";
			$db->setQuery($sql);
			$wg_heros=NULL;
			$wg_heros=$db->loadObjectList();
			$sum_hero=count($wg_heros);
			if($wg_heros)
			{
				if(count($_POST)<>0)
				{
					if($_POST['rank_hero']=="" && $_POST['value_name']=="")
					{
						header("Location:build.php?id=".$building->index."&t=4");
						exit();
					}		
					elseif($_POST['rank_hero']!="")
					{	
						if(!is_numeric($_POST['rank_hero']) || $_POST['rank_hero'] <1 )
						{
							header("Location:build.php?id=".$building->index."&t=4");
							exit();
						}
						...
						{
							if($_POST['rank_hero'] > $sum_hero)
							{
								$rank=$sum_hero;
								$parse['rank_hero']=$sum_hero;						
							}
							else
							{	
								$rank=$_POST['rank_hero'];
								$parse['rank_hero']=$_POST['rank_hero'];						
							}
					...
					{
						$parse['error']=''.$lang['error1d'].' <strong>[ '.$_POST['value_name'].' ]</strong> '.$lang['error2'].'';
						$rank=1;
						foreach($wg_heros as $key=>$v)
						{
							if($_POST['value_name']==$v->username)
							{
								$rank=$key+1;
								$parse['error']='';
								$parse['value_name']=$_POST['value_name'];
								break;						
							}
						}
						if($rank % constant("MAXROW")==0)
				...
				{ 	
					...
					elseif($_GET["page"]<1 || ($_GET["page"]-1)*constant("MAXROW")>$sum_hero)
					{
						header("Location:build.php?id=".$building->index."&t=4");
						exit();
					}
				...
				foreach($wg_heros as $key =>$v)
				{
					if($key >=$x && $key <=$y)
					{
						$parse['rank']=$key+1;
						$parse['name']=$v->name;	
						$parse['troop_name']=$lang[$wg_troops[$v->troop_id-1]['name']];	
						$parse['image']=$wg_troops[$v->troop_id-1]['image'];	
						$parse['player']=$v->username;	
						$parse['exprience']=$v->kinh_nghiem;	
						$parse['level']=$v->level;	
						$parse['id2']=$v->user_id;
						if($key==$rank-1) 
						{
						...
						$list.= parsetemplate(gettemplate('ranking_hero_row'),$parse);
					}
				}
				$a="'build.php?id=".$building->index."&t=4&page='+this.options[this.selectedIndex].value";
				$b="'_top'";
				...
				$parse['page'].=getPages($_GET["page"],$sum_hero,$p);
				$parse['list']=$list;
				$parse['task_content']= parsetemplate(gettemplate("ranking_hero"), $parse);
			}	
			break;
		default:
			...
			if($hero)
			{
				...
				if($hero->village_id==$wg_village->id)
				{
					...
					$parse['image']=$wg_troops[$hero->troop_id - 1]['image'];
...
function udpData($vId, $palaceLevel){
	...
	if($objVillageRes[0]->faith < 100){//When the faith small than 100
		...
		$deltaFaith = $time_distance * (($palaceLevel+1) * ($speed/20))/ 86400;
...
//@Author: DiepLuan
function GetUserAli_id($userID)
{
	global $db;
	$sql = "SELECT `ally_id` FROM `wg_ally_members` WHERE `user_id`=$userID and right_ = 1";
	$db->setQuery($sql);
	$userAli_id = $db->loadResult();
	return $userAli_id;
}
function GetLevelAli($AliID)
{
	global $db;
	$sql = "SELECT rank FROM `wg_allies` WHERE `id` =$AliID";
	$db->setQuery($sql);
	$level = $db->loadResult();
	return $level;
}
