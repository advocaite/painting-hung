...
function checkcookies()
{
    ...
    if (isset($_COOKIE[$game_config['COOKIE_NAME']]))
    {
        ...
        if (!$row)
        {
            ...
            $row = false;
            globalError3 ("Hacking!!! cookies khong hop le");
        }
...
function message($mes, $title = 'Error', $dest = "", $time = "3")
{
    ...
    display($page, $title, false,
            false, (($dest != '') ? "<meta http-equiv=\"refresh\" content=\"$time;URL='$dest';\">" : ''));
}
...
function display_newlogin($page, $title = '', $topnav = true, $metatags = '')
{
    global $link, $game_config, $debug, $user;
    echo "$page";
...
{
    global $db, $user, $lang, $timeAgain, $game_config;
    ...
    if (check_user())
    {
        ...
        $sql             = "SELECT id FROM wg_messages WHERE id_user=" . $user["id"]
            . " AND from_id >0 AND status=0 LIMIT 1";
        $db->setQuery($sql);
        $wg_messages = NULL;
        ...
        $parse['show_time_gamer'] = '<span style="float: right;padding-right:15px;">' . $lang['full_times'] . '</span>';

        if ($timeAgain > 0)
        {
            $parse['show_time_gamer'] = '<span style="float: right;padding-right:15px;">' . $lang['time_gamer']
                . '&nbsp;<span id="limit1" class="b">' . ReturnTime($timeAgain) . '</span></span>';
        }
...
function InsertReport($uer_id, $title, $time, $report_text, $type = 0, $uer_id_2 = 0)
{
    ...
    $sql         =
        "INSERT INTO wg_reports (`user_id`, title, time, report_text, type, status) VALUES ($uer_id, '$title', '$time', '$report_text', $type, 0)";
    $db->setQuery($sql);
    ...
    $sql_bk =
        "INSERT INTO wg_reports_bk (user_id, `user_id_2`, title, time, report_text, type, status) VALUES ($uer_id, '$uer_id_2', '$title', '$time', '$report_text', $type, 0)";
    $db->setQuery($sql_bk);
 ...
 * them mot status.
 */
function InsertStatus($village_id, $object_id, $time_begin, $time_end, $cost_time, $type, $order = 0, $level = 0)
{
    global $db;
    $sql =
        "INSERT INTO wg_status (`village_id`, `object_id`, `type`, `time_begin`, `time_end`, `cost_time`, `status`, `order_`,`level`) VALUES ($village_id, $object_id, $type, '$time_begin', '$time_end', $cost_time, 0, $order,$level)";
    $db->setQuery($sql);
...
function GetRallyPoint($village_id)
{
    ...
    $buildingTypeID = 27;
    $sql            =
        "SELECT id, wg_buildings.index FROM wg_buildings WHERE vila_id=$village_id AND type_id=$buildingTypeID";
    $db->setQuery($sql);
    $building = null;
...
/**
 * Tao link tu m?t lang toi rally point de send troop
 */
function LinkSendTroop($village_id, $village_defend_x, $village_defend_y)
{
    ...
    {
        return
            "<div class=\"nbr\"><a href=\"build.php?id=$rally->index&t=1&vdx=$village_defend_x&vdy=$village_defend_y\">&raquo; "
            . $lang['send_troop_link'] . "</a></div>";
    }
    else
    {
        return "";
...
function LinkSendMerchant($village_to_x, $village_to_y)
{
    ...
    if ($market)
    {
        return "<div class=\"nbr\"><a href=\"build.php?id=$market->index&vtx=$village_to_x&vty=$village_to_y\">&raquo; "
            . $lang['send_merchant_link'] . "</a></div>";
    }
    else
    {
        return "";
...
function setFlagStatus($value)
{
    global $db;
    $sql = "UPDATE wg_config SET `config_value`='$value' WHERE `config_name`='flag_status'";
    $db->setQuery($sql);
...
function doAllStatus()
{
    ...
    require_once ('function_profile.php');
    ...
    //{
        ...
        $sSql   = "SELECT * FROM wg_status WHERE UNIX_TIMESTAMP(time_end) <= UNIX_TIMESTAMP('$ymdhis')";
        $sSql .= " AND status=0 ORDER BY time_end ASC ";
        $db->setQuery($sSql);
        $objResSts = $db->loadObjectList();

        if (count($objResSts))
        {     //Neu co du lieu
            foreach ($objResSts as $k => $v)
            { // Thuc hien vong lap cho tung loai
                //update wg_status : cho biet la da xu ly roi
                executeUpdateStatus ($v->id);
                ...
                if ($db->getAffectedRows())
                {
                    ...
                    switch ($type)
                    {           // Ung voi moi type
                        case 1: // Xây d?ng building
                            try
                            {
                                ConstructBuilding($v->object_id, $v->village_id, $v->type, $v->order_);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 2: // Nâng c?p nhà trong
                            try
                            {
                                updateBuilding($v->object_id, $v->village_id, $v->level, $v->order_);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 3: // Nâng c?p nhà ngoài
                            try
                            {
                                updateResourceOutSide($v->object_id, $v->village_id, $v->level, $v->order_);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 4:  // tao linh trong doanh trai
                        case 12: // tao linh trong trai ngua
                        case 13: // tao linh trong cung dien
                        case 20: // tao linh trong con xuong
                            try
                            {
                                executeCreateTroop ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 6:  // van chuyen ( Mua, Ban)
                        case 23: // Send resource(Giao d?ch)
                            try
                            {
                                executeResourceBuySale ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 10: // linh thang tran tro ve
                            try
                            {
                                executeAttackReturn ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 14: // nang cap giap
                            try
                            {
                                executeUpClothings ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 15: // tho di tao lang moi
                            include_once ("function_foundvillage.php");

                            try
                            {
                                executeSendWorkers ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 16: // chien loi pham ve lang
                            try
                            {
                                executeAttackGetResource ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 17: // Phá h?y công tr?nh
                            try
                            {
                                destroyBuilding($v->object_id, $v->village_id, $v->level, $v->order_);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 18: // Delete acc
                            try
                            {
                                executeAccountDelete ($v->object_id);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 21: // hoi sinh hero
                            try
                            {
                                executeHeroComeback ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 22: // thuong nhan ve thanh
                            try
                            {
                                executeTradeComeback ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 24: // Chuyen bau vat
                            try
                            {
                                executeSentRareOff ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 25: // Dua bau vat chiem duoc ve thanh
                            try
                            {
                                executeAttackGetRare ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 26: // di chuyen linh trong cong thanh chien
                            include_once ("function_ctc.php");

                            try
                            {
                                executeSendTroop_ctc ($v);
                            }
                            catch (Exception $e)
                            {
                                globalError2 ($e);
                        ...
                        case 27: // attack cong thanh chien
                            include_once ("function_ctc.php");
                        ...
                        default: break;
                    } // End switch Ung voi moi type
                }
            }         // End foreach Thuc hien vong lap cho tung loai
        }             //Endif Neu co du lieu
...
function deleteVillage($villageId)
{
    ...
    //wg_villages_map
    $sql = "UPDATE wg_villages_map SET `workers` = '2', `user_id`='0', `faith`='100' WHERE id = '" . $villageId . "'";
    $db->setQuery($sql);
    ...
    //wg_resource_sends
    $sql = "DELETE FROM wg_resource_sends WHERE village_id_from = '" . $villageId . "' OR village_id_to = '"
        . $villageId . "'";
    $db->setQuery($sql);
    ...
    //wg_attack:
    $sql = "SELECT * FROM wg_attack WHERE village_attack_id = '" . $villageId . "' OR village_defend_id = '"
        . $villageId . "'";
    $db->setQuery($sql);
    ...
    if ($attackList)
    {
        ...
        {
            $sql = "DELETE FROM wg_attack_troop WHERE attack_id = '" . $attack->id . "'";
            $db->setQuery($sql);
        ...
        $sql = "DELETE FROM wg_attack WHERE village_attack_id = '" . $villageId . "' OR village_defend_id = '"
            . $villageId . "'";
        $db->setQuery($sql);
