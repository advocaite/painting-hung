...
global $wg_village,$db,$wg_buildings,$timeAgain,$user,$wordFilters;
...
if(isset($_POST['delete']))
{
	...
	if(isset($arrs))
	{
		...
		{	
			if(is_numeric($arr))
			{
				$sql = "delete from wg_messages where id='".$db->getEscaped($arr)."' and id_user=".$user["id"]."";
				$db->setQuery($sql);
				$db->query();	
			}							
		}								
...
if(empty($_GET['tab']) ||$_GET['tab']!=1)
{
	...
	else
	{
		...
		foreach ($list_messages as $ptu)
		{
			...
			$parse['time']=substr($ptu->times,0,10).' '.substr($ptu->times,8,2).'-'.substr($ptu->times,5,2).'-'.substr($ptu->times,2,2);
			$list .= parsetemplate($row, $parse);	
		}
		$parse['list']=$list;
		/*------------------Phan trang --------------------------------------*/	
	...
	if(!empty($_GET['id']) && is_numeric($_GET['id']))
	{
			...
			if(count($_POST)<>0)
			{
				...
				if($_GET['act']=='answer')
				{
					...
					{	
						...
						$content=substr($db->getEscaped(strip_tags($_POST['content'])),0,5000);
						foreach($wordFilters as $a => $b)
						{
							$content = str_replace("$a",'*', $content);		
							$topic = str_replace("$a",'*', $topic);					
						}
		...
		(".$user['id'].",0,".$to_id.",'".date("Y-m-d H:i:s")."',0,'".$topic."','".$content."'),(".$to_id.",".$user['id'].",0,'".date("Y-m-d H:i:s")."',0,'".$topic."','".$content."')";
			...
			else
			{
				...
				$sql="UPDATE wg_messages SET status=1 WHERE id=".$id."";
				$db->setQuery($sql);
				$db->query();
				if($query->from_id>0)
				{
