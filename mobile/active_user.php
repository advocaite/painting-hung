<?php 
/* UPDATE `wg_registration_village_list` SET `registed`=1 WHERE `zone_id`=1 */
session_start();
ob_start();
define('INSIDE', true);
$ugamela_root_path = './';
...
if($_POST['active'])
{
	...
	{
		header("Location:http://asu.ingame.vn/Default.aspx?mod=reg");
		exit();	
	}
...
{			
	...
	if(!$actived)
	{
		if(active_remote($username,$code))
		{
			...
			capPhat($username,$nation,$position);
