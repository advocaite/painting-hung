	...
	foreach ($wg_buildings as $key => $ptu)
	{
		if($ptu->index >=18)
		{
			if($i<38)
			{
				...
				if($ptu->type_id == 27)
				{
					$parse['image'].="<br/><p><a href='general_more.php?id=".$ptu->index."&amp;t=1'>".$lang[$ptu->name]."</a> ($ptu->level)</p>";
				}
				//show nhung cong trinh trong phan tong quat
				elseif($ptu->type_id == 18 OR $ptu->type_id == 24 OR $ptu->type_id == 28 OR $ptu->type_id == 29 OR $ptu->type_id == 31 OR $ptu->type_id == 30 OR $ptu->type_id == 35)
				{
					$parse['image'].="<br/><p><a href='general_more.php?id=".$ptu->index."'>".$lang[$ptu->name]."</a> ($ptu->level)</p>";				
				}
			}			
			$j++;
			$parse['build_'.$i.'']="general_more.php?id=".$ptu->index."";
			if($ptu->level>0)
			{
...
//hien thi quan doi trong lang
$parse['list_troop_in_village'] = showTroopInVillage();
//hien thi quan tiep vien
$parse['list_troop_reinforce'] = showTroopReinforceInVillage();
//hien thi tinh hinh chien su
