...
global $db,$user,$wg_village,$wg_buildings,$village,$Main_time,$wg_buildings,$timeAgain;
includeLang('build');
...
if(isset($_REQUEST['id']) && is_numeric($_REQUEST['id']))
{
		...
		elseif($_GET_ID >18 && $_GET_ID <=38)
		{
			...
			elseif($wg_buildings[$_GET_ID-1]->type_id == 0 )
			{
				...
				else
				{
					...
					{
						$parse['list building'].=Buil_Infrastructure_New(1,$user["id"],15,$_REQUEST['id'],$village,$a,$b,$c,$d,$Main_time);
					}
				...
				$parse['out_village']="Ng_T";
				$parse['Village center']="N_T";
				$parse['overview']="TQ";
				$parse['Asu']="Asu";
			...
			{
				$parse['build_name_and_level'] = Update_Resource_Buidling_ForWAP1(2,$_GET_ID,$user["id"],$_GET_ID-1,$village,$Main_time);
				$parse['body_content'] = Update_Resource_Buidling_ForWAP2(2,$_GET_ID,$user["id"],$_GET_ID-1,$village,$Main_time);
				...
				$page = parsetemplate(gettemplate('general_more'),$parse);
				display($page,$lang['Title']);
			} 	
		}
	}
	...
	{
		header("Location:general_more.php");
		exit();
	}	
}
/*-------------------------------------------------------------------------------------------------------------------*/
