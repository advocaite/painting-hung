...
function destroyByCata($id,$type_id,$level,$village)
{
	...
	$cp=getCpAll($type_id,$level-1);
	$product_hour_new=getProductNew($type_id,$level-1);
	$string='';
	$image=Get_Images2($type_id,$level);
	
	if($type_id==6)//"Sawmill")
	{
		$ct=(1+($level-1)*0.05)/(1+$level*0.05);
		$string='krs1=ROUND(krs1*'.$ct.',6),';
	}
	if($type_id==7)//"Brickyard")
	{
		$ct=(1+($level-1)*0.05)/(1+$level*0.05);
		$string='krs2=ROUND(krs2*'.$ct.',6),';
	}
	if($type_id==8)//"Iron_Foundry")
	{
		$ct=(1+($level-1)*0.05)/(1+$level*0.05);
		$string='krs3=ROUND(krs3*'.$ct.',6),';
	}
	if($type_id==5 ||$type_id==9)//"Bakery" "Grain_Mill"
	{
		$ct=(1+($level-1)*0.05)/(1+$level*0.05);
		$string='krs4=ROUND(krs4*'.$ct.',6),';
	}
	if($level-1<=0)
	{
		if($type_id<=4)
		{
			$sql="UPDATE wg_buildings SET level=0,product_hour=$product_hour_new,cp=0 WHERE id=$id";
			$db->setQuery($sql);
			$db->query();
		...
		{
			if($type_id==14) //embassy ->cap nhat trang thai da chua  ton tai embassy trong wg_user 
			{				
	...
	{
		$sql="UPDATE wg_buildings SET img='$image',level=level-1,product_hour=$product_hour_new,cp=$cp WHERE id=$id";
		$db->setQuery($sql);
		$db->query();
		if($db->getAffectedRows()==0)
		{
	...
	if($Workers_new>0)
	{
		$sql="UPDATE wg_villages SET ".$string."workers=workers-$Workers_new WHERE id=".$village."";
		$db->setQuery($sql);
		$db->query();
		if($db->getAffectedRows()==0)
		{
		...
		$sql="UPDATE wg_users SET population=population-$Workers_new 
		WHERE id=(SELECT user_id FROM wg_villages WHERE id=".$village.")";
		$db->setQuery($sql);
		$db->query();
		if($db->getAffectedRows()==0)
		{
...
function ConstructBuilding($index,$village,$type_status,$userid)
{
	...
	if($type_id==6)//"Sawmill")
	{
		$string='krs1=ROUND(krs1*1.05,6),';
	}
	elseif($type_id==7)//"Brickyard")
	{
		$string='krs2=ROUND(krs2*1.05,6),';
	}
	elseif($type_id==8)//"Iron_Foundry")
	{
		$string='krs3=ROUND(krs3*1.05,6),';
	}
	elseif($type_id==5 || $type_id==9)// Grain_Mill //Bakery
	{
		$string='krs4=ROUND(krs4*1.05,6),';		
	}	
	$sql="UPDATE wg_buildings SET img='$images',level=1,product_hour=$product_hour_new,cp=$cp WHERE id=$id";
	$db->setQuery($sql);
	$db->query();
	if($db->getAffectedRows()==0)
	{
...
function updateResourceOutSide($level,$index,$village,$userid)
{
	global $db,$game_config;
	$Hs_K=$game_config['k_game'];
	$sql="SELECT id,type_id FROM wg_buildings WHERE `index`=$index AND vila_id=$village";
	...
	$sql="UPDATE wg_buildings SET level=level+1,product_hour=$product_hour_new,cp=$cp WHERE id=".$id;
	$db->setQuery($sql);
	$db->query();
	if($db->getAffectedRows()==0)
	{
...
function updateBuilding($index,$village,$level,$userid)
{
	...
	if($type_id==6)//"Sawmill")
	{
		$ct=(1+$level*0.05)/(1+($level-1)*0.05);
		$string='krs1=ROUND(krs1*'.$ct.',6),';
	}
	elseif($type_id==7)//"Brickyard")
	{
	...
	elseif($type_id==37 && $level>10) // tao linh am binh
	{
	...
	$sql="UPDATE wg_buildings SET img='$images',level=level+1,product_hour=$product_hour_new,cp=$cp WHERE id=$id";
	$db->setQuery($sql);
	$db->query();
	if($db->getAffectedRows()==0)
	{
...
function destroyBuilding($index,$village,$level,$userid)
{
	...
	{
		$levelA=$level+1;
		$ct=(1+($levelA-1)*0.05)/(1+$levelA*0.05);
		$string='krs1=ROUND(krs1*'.$ct.',6),';
	}
	elseif($type_id==7)//"Brickyard")
	{
	...
	if($level==0)
	{
		if($type_id==14) //embassy
		{
			...
			$sql="UPDATE wg_users  SET embassy=0 WHERE id=".$userid." LIMIT 1";
			$db->setQuery($sql);
			$db->query();
			if($db->getAffectedRows()==0)
			{
		...
		$sql="UPDATE wg_buildings SET name='',img='',type_id=0,level=0,product_hour=0,cp=0 WHERE id=$id LIMIT 1";
		$db->setQuery($sql);
		$db->query();
		if($db->getAffectedRows()==0)
		{
	...
	{
		$sql="UPDATE wg_buildings SET level=$level,product_hour=$product_hour_new,cp=$cp WHERE id=$id LIMIT 1";
		$db->setQuery($sql);
		$db->query();
		if($db->getAffectedRows()==0)
		{
	...
	if($Workers_new>0)
	{	
		$sql="UPDATE wg_villages SET ".$string."workers=workers-$Workers_new WHERE id=".$village." LIMIT 1";
		$db->setQuery($sql);
		$db->query();
		if($db->getAffectedRows()==0)
		{
		...
		$sql="UPDATE wg_users SET population=population-$Workers_new WHERE id=".$userid." LIMIT 1";
		$db->setQuery($sql);
		$db->query();
		if($db->getAffectedRows()==0)
		{
...
function List_Invite($userid)
{
	...
	{	
		...
		{
			$sql="SELECT id,tag FROM wg_allies WHERE id=".$result->ally_id."";
			$db->setQuery($sql);
			$query=null;
			$db->loadObject($query);
			$parse['id1']=$result->id;
...
function Update_Resource_Buidling($type,$id,$userid,$stt,$village,$Main_time)
{
	...
	else
	{
		...
		{
			$dieu_kien=Demolish_destroy($village,$Main_time,$Hs_K,$userid,$wg_status);
		}
		else
		{
...
function returnWorkersLogin($userid)
{
	...
	if($wg_villages)
	{
		...
		{
			$sum+=returnWorkersVillage($result->id);							
		}
...
function returnWorkersVillage($villages_id)
{
	global $db,$game_config;
	$sql="SELECT id,`index`,type_id,level,product_hour FROM wg_buildings WHERE vila_id=".$villages_id." ORDER BY `index` ASC";
	$db->setQuery($sql);
	$wg_villages=NULL;
	$wg_villages=$db->loadObjectList();
	$sum=0;
	if($wg_villages)
	{
		foreach($wg_villages as $key=>$value)
		{
			...
			$sql="UPDATE wg_buildings SET product_hour=".$product_hour." WHERE id=".$value->id;
			$db->setQuery($sql);
			$db->query();
		}
	}	
	$sql="UPDATE wg_villages SET workers=".$sum." WHERE id=".$villages_id;
	$db->setQuery($sql);
	$db->query();
	return $sum;
}
/*-------------------------------------------------------------------------------------------------------------------------------
* @Author: duc hien
* @Des:	ham dung de nang cap tai nguyen trong va ngoai theo level  (For WAP)
* @param: $type= trang thai type trong status
$id=so thu tu trong bang wg_buidling
* @return: 
*/
/*----------------------------------------------------------------------------------------------------------------------*/
function Update_Resource_Buidling_ForWAP1($type,$id,$userid,$stt,$village,$Main_time)
{
	global $db,$user,$lang,$game_config,$wg_buildings,$wg_village,$wg_building_types,$wg_status;
	$parse = $lang;
	$Hs_K=$game_config['k_game'];
	...
	else
	{
		...
		$parse['dieu_kien']=$dieu_kien;
		$parse['need_worker']=$need_worker;
		$content_update=parsetemplate(gettemplate('general_build_update'),$parse);	
	}
	return $content_update;
}
function Update_Resource_Buidling_ForWAP2($type,$id,$userid,$stt,$village,$Main_time)
{
	global $db,$user,$lang,$game_config,$wg_buildings,$wg_village,$wg_building_types,$wg_status;
	$parse = $lang;
	$Hs_K=$game_config['k_game'];
	...
	else
	{
		...
		$parse['dieu_kien']=$dieu_kien;
		$parse['need_worker']=$need_worker;
		$content_update=parsetemplate(gettemplate('general_more_build_update'),$parse);	
	}
	return $content_update;
}
