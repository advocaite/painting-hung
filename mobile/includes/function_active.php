 ...
 */
function capPhat($username,$nation_id,$position)
{
	...
	if($village_map_id)
	{
		...
		$user_id=insertWg_users($username,$village_map_id,$position,$nation_id);		
		insertPlus($user_id);
...
function insertWg_users($username,$village_id,$position,$nation)
{
	global $db;	
	$sql="INSERT INTO `wg_users` (`username`,`position`,`villages_id`,`sum_villages`,`population`,`nation_id`,`active_time`,`actived`) VALUES ('".$username."',".$position.",".$village_id.",1,2,".$nation.",'".date("Y-m-d H:i:s")."',1)";
	$db->setQuery($sql);
	if(!$db->query())
	{
...
function GetVillageForNewUser($position)
{
	...
	{
		$sql="SELECT village_id FROM wg_registration_village_list WHERE zone_id=$position AND registed=0 ORDER BY id ASC LIMIT 0,200";
		$db->setQuery($sql);
	...
	{
		$sql="SELECT village_id FROM wg_registration_village_list WHERE zone_id=$position AND registed=0 ORDER BY id ASC";
		$db->setQuery($sql);
...
function KhoiTaoVillage($village_map_id, $user_id, $user_name='', $nation, $name=''){
	...
	if($villageMap){
		...
		if($db->query()){
			/*-------------------------- code cua anh trung -----------------------------------------------*/
			$sql="UPDATE `wg_villages_map` SET `workers` = '2',user_id=$user_id WHERE `wg_villages_map`.`id` =$village_map_id";
			$db->setQuery($sql);
			$db->query();
			return $kind_id;
		}else{
...
function KhoiTaoTroopResearch($village_id, $nation_id){
	...
	//$tho_id=GetTroopIDByName($nation_id, "Thợ");
