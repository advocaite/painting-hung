...
function listVillageAndAllyMember($userid,$village)
{
	...
	{// user do khong co dong minh */
		...
		$db->setQuery($sql);
		$elements = $db->loadObjectList();
		$parse['List my alliance']='<table class="f10">';			
		if($elements)
		{
			foreach ($elements as $ptu)
			{
				$parse['List my alliance'].='			
	      		<tr>
					<td class="nbr"><span class="c2">&#8226;</span>&nbsp; 
						<a href="profile.php?uid='.$ptu->user_id.'" class="active_vl">'.$ptu->username.'</a>
					</td>
				</tr>';
			}
		}
		$parse['List my alliance'].='</table>';
	}	
	$parse['account_delete_status'] = "";
...
function getInvitationList($allyId)
{
	...
	includeLang('allianz');
	$parse = $lang;
	$sql = "SELECT id,user_id FROM wg_ally_members WHERE right_ = 0 AND ally_id = ".$allyId." ORDER BY id DESC";
	$db->setQuery($sql);	
	...
	if($invitations){
		foreach($invitations as $invitation){
			$userName = getUserNameByUserId($invitation->user_id);
			$userId = $invitation->user_id;
			$id = $invitation->id;
			if(empty($userName)){
				$parse['invitation'] = "<tr>none</tr>";				
			}else{
				$parse['invitation'] = "
				
				<tr>
					 <td><a href=\"allianz.php?del_invite=$id\">
						<img src=\"images/un/a/del.gif\" width=\"12\" height=\"12\" border=\"0\"></a>
					 </td>
					 <td class=\"s7\"><a href=\"profile.php?uid=$userId\">".$lang['Invitation for']." $userName</a>
					 </td>
				</tr>";
			}
			$viewInvitationList .= parsetemplate(gettemplate('allian_invite_player_list'),$parse);
			$countInvitation++;
		}
	}
