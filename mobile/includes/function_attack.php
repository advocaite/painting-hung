	...
	if($sumFeudation<$maxFeudation){
		...
		if($oasis->faith<35){
			...
			if($oasis->user_id){
				...
				$villageReinforce=getVillage($mainVillageObj->child_id, "id, krs1, krs2, krs3, krs4");
...
function phaseAttWonder($sumAtt, $sumDf, $sumAttTrp, $sumDfTrp){
	...
	if($sumAtt>0 && $sumDf>0){
		if($sumAtt > $sumDf){
			$y=$sumDf/$sumAtt;
			$dfDie	=$sumDfTrp;
			$attDie	=min(round($dfDie*$y, 0), $sumAttTrp);
		}else{
			$y=$sumAtt/$sumDf;
			$attDie	=$sumAttTrp;
			$dfDie=min(round($sumAttTrp*$y, 0), $sumDfTrp);
		}
	}
