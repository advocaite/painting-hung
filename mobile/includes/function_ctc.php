 ...
 */
function getPostPheID_ctc(){
	global $db;
	$ct_id = intval($_REQUEST['ct']);
	$cong_thu = intval($_REQUEST['p']);
	
	$sql = "SELECT * FROM wg_ctc_phe WHERE ct_id=$ct_id AND cong_thu=$cong_thu";
	$db->setQuery($sql);
	$db->loadObject($rs);
	return $rs;
}
...
//	echo "<pre>"; print_r($ats);die();
//	echo $dtk->id.":".$sa." - ".$sd."<br>";
...
//	backUpTroop_ctc($ats, $dtk->id, $luot);
//	backUpTroop_ctc($dts, $dtk->id, $luot);
//	backUpHero_ctc($ahrs, $dtk->id, $luot);
//	backUpHero_ctc($dhrs, $dtk->id, $luot);
	
	//Tao report:
	report_ctc($dtk->id, $luot, $ats, $dts, $ahrs, $dhrs, $art);
}
 ...
 * 		$luot:	luot danh
 */
function backUpTroop_ctc($vts, $dtk_id, $luot){
	global $db;
	if($vts){
 ...
 */
function report_ctc($dtk_id, $luot, $vats, $vdts, $ahs, $dhs, $arrayTroop){
	global $db, $lang;
	$parse = array();
	$parse = $lang;
	for($i=1; $i<=33; $i++){
...
/**
 * @author Le Van Tu@todo
 * @todo Lay danh sach ali chua mot phe
 */
 ...
 */
function getUserSide_ctc($dtk_id, $sid){
	global $db;
	$sql = "SELECT
					wg_users.id,
					wg_users.username
...
function getReport_ctc($dtk_id){
	global $db, $lang;
	$rs = "";
	$d = date("Y-m-d");
	$sql = "SELECT * FROM wg_ctc_report WHERE dtk_id='$dtk_id' AND date_attack='$d'";
	$db->setQuery($sql);
...
function end_ctc(){
	...
	//vo hieu cac the cong thanh chien:
	destroyCard_ctc();
	
	//bo phe cua cac lien minh
	//destroyAliSide_ctc();
}
 ...
 */
function returnTroop_ctc(){
	global $db;
	//tra tuong trong diem tap ket
	$sql = "SELECT wg_ctc_heros.village_id, wg_ctc_heros.hero_id FROM wg_ctc_heros";
	$db->setQuery($sql);
	$hs = $db->loadObjectList();
	if($hs){
		foreach($hs as $h){
		...
		$sql = " TRUNCATE TABLE `wg_ctc_heros`";
		$db->setQuery($sql);
		if(!$db->query()){
			globalError2($sql);
		}
	...
	$sql = "SELECT wg_ctc_troops.village_id, wg_ctc_troops.troop_id, wg_ctc_troops.num FROM wg_ctc_troops";
	$db->setQuery($sql);
	$ts = $db->loadObjectList();
	if($ts){
		foreach($ts as $t){
		...
		$sql = " TRUNCATE TABLE `wg_ctc_troops`";
		$db->setQuery($sql);
		if(!$db->query()){
			globalError2($sql);
		}
	...
	//tra tuong dang di chuyen:
	$sql = "SELECT
					wg_ctc_hero_onway.hero_id,
					wg_status.village_id
				FROM
					wg_ctc_hero_onway ,
					wg_ctc_send_troop ,
					wg_status
				WHERE
					wg_ctc_hero_onway.st_id =  wg_ctc_send_troop.id AND
					wg_ctc_hero_onway.`status` =  '0' AND
					wg_ctc_send_troop.`status` =  '0' AND
					wg_status.`status` =  '0' AND
					wg_status.object_id =  wg_ctc_send_troop.id
				GROUP BY
					wg_ctc_hero_onway.id";
	$db->setQuery($sql);
	$hs = $db->loadObjectList();
	if($hs){
		foreach($hs as $h){
		...
		//set status=1 trong bang wg_ctc_hero_onway:
		$sql = "UPDATE `wg_ctc_hero_onway` SET wg_ctc_hero_onway.status=1 WHERE wg_ctc_hero_onway.status=0";
		$db->setQuery($sql);
		if(!$db->query()){
			globalError2($sql);
		}
	...
	//tra linh dang di chuyen:
	$sql = "SELECT
						wg_status.village_id,
						wg_ctc_troop_onway.troop_id,
						wg_ctc_troop_onway.num
					FROM
						wg_ctc_send_troop ,
						wg_status ,
						wg_ctc_troop_onway
					WHERE
						wg_ctc_troop_onway.st_id =  wg_ctc_send_troop.id AND
						wg_status.object_id =  wg_ctc_send_troop.id AND
						wg_ctc_troop_onway.`status` =  '0' AND
						wg_ctc_send_troop.`status` =  '0' AND
						wg_status.`status` =  '0'
					GROUP BY
						wg_ctc_troop_onway.id";
	$db->setQuery($sql);
	$ts = $db->loadObjectList();
	if($ts){
		foreach($ts as $t){
		...
		//cap nhat status=1 trong bang wg_ctc_send_troop va wg_ctc_troop_onway:
		$sql = "UPDATE `wg_ctc_send_troop` SET wg_ctc_send_troop.status=1 WHERE wg_ctc_send_troop.status=0";
		$db->setQuery($sql);
		if(!$db->query()){
			globalError2($sql);
		}
		
		$sql = "UPDATE `wg_ctc_troop_onway` SET wg_ctc_troop_onway.status=1 WHERE wg_ctc_troop_onway.status=0";
		$db->setQuery($sql);
		if(!$db->query()){
			globalError2($sql);
		}
	...
	//cap nhat status=1 cho nhung status gui quan trong ctc:
	$sql = "UPDATE `wg_status` SET wg_status.status=1 WHERE wg_status.status=0 AND wg_status.type='26'";
	$db->setQuery($sql);
	if(!$db->query()){
		globalError2($sql);
	}
 ...
 */
function destroyCard_ctc(){
	global $db;
	$sql = "UPDATE wg_plus SET the_bai_1=0, the_bai_2=0, the_bai_3=0";
	$db->setQuery($sql);
	if(!$db->query()){
		globalError2($sql);
	}
...
function getPointTable_ctc($ct_id){
	...
	$sql = "SELECT wg_status.time_end, wg_status.order_ FROM wg_status WHERE wg_status.`type`  = '27' AND wg_status.object_id = '$ct_id' ORDER BY wg_status.time_end DESC LIMIT 0, 10";
	$db->setQuery($sql);
	$ts = $db->loadObjectList();
	if($ts){
		foreach($ts as $t){			
	...
	if($ps){
		foreach($ps as $p){
			if($rs['at_p_'.$p->luot]=="" && $rs['df_p_'.$p->luot]==""){
				$rs['at_p_'.$p->luot] = 0;
				$rs['df_p_'.$p->luot] = 0;
			}
...
function displayTroopInDTK_ctc($dtk_id){
	...
	$rs = array();
	for($i=1; $i<=33; $i++){
		$rs['dis_a_'.$i] = 'style="display:none"';
		$rs['dis_d_'.$i] = 'style="display:none"';
	}
