...
function getIndexOfPalace($village_id)
{
	global $db;
	$value=NULL;
	$sql="SELECT `index` FROM wg_buildings WHERE vila_id = ".$village_id." AND type_id = 18"; 
	$db->setQuery($sql);
	$value=$db->loadResult();	
	return $value;
}
...
function getIndexOfHeroMansion($village_id)
{
	global $db;	
	$value=NULL;
	$sql="SELECT `index` FROM wg_buildings WHERE vila_id = ".$village_id." AND type_id = 35"; 
	$db->setQuery($sql);	
	$indexHeroMansion = null;
	$value=$db->loadResult();	
	return $value;
}
