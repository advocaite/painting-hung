...
function reportTransport($status, $rsSend){
	...
	InsertReport($villageFrom->user_id, $title, date("Y-m-d H:i:s"), $content, REPORT_TRADE);
	if($villageFrom->user_id!=$villageTo->user_id){
		InsertReport($villageTo->user_id, $title, date("Y-m-d H:i:s"), $content, REPORT_TRADE_RECEIVE);
	}		
}
