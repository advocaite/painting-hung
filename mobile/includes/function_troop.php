...
function Train($building, $status_type, $maxPalaceTrain=null){
	...
	if(isset($_REQUEST['t1'])){
		...
		header("Location: general_more.php?id=$building->index");
	}
	...
	if($building->type_id==18){//nha nay la palace
		...
		if($maxTroopTrain['tho']>0 || $maxTroopTrain['thuyet_gia']>0){
			...
			$parse['list_troops']=parsetemplate(gettemplate("train_table_null"), $parse);
		}
	}else{
	...
	if(count($listTroops)>0){
		...
		foreach($listTroops as $troop){
			...
			if($_REQUEST['t'.$i]>0 && $troop->sum>0){
				...
				if($max>0){
					$sumTrain=$_REQUEST['t'.$i]>$max?$max:$_REQUEST['t'.$i];
...
function rutQuanVienTro($building){
	...
	if(count($_REQUEST)>0){
		...
		$listAttackTroop=GetListAttackTroop($_REQUEST['aid']);
		$heroAttack=GetAttackHero($_REQUEST['aid']);
		if($heroAttack && $_REQUEST['h']==1){
			$coHero=true;
		}else{
			$coHero=false;
		}
	...
	header("Location: general_more.php?id=37");
}
...
function ShowTroopComeBack(&$timer){
	...
	if($attackList){
		...
		foreach($attackList as $attack){
			...
			$parse['time_at']=Date("H:i:s", strtotime($attack->time_end));
			$parse['date_at']=Date("d-m-Y", strtotime($attack->time_end));			
			$comeBack .= parsetemplate(gettemplate("list_troop_status"), $parse);
		}
	}
	return $comeBack;	
}
...
function SendTroops($building){
	...
	if(isset($_REQUEST['vn'])){
		if($_REQUEST['vn']){
			...
			$villageDefend=CheckVillageName($_REQUEST['vn']);
			...
			if(is_numeric($_REQUEST['x']) && is_numeric($_REQUEST['y'])){
				$villageDefend=checkVillageLocationSendTroop($_REQUEST['x'], $_REQUEST['y']);
				if(!$villageDefend){
		...
		if($villageDefend->kind_id<7){
			...
			if(($privateTime=isPrivateVillage($villageDefend->id)) && $_REQUEST['st']!=2){
		...
		if($villageDefend){
			...
			if($user->anounment!="ban"){//Kiem tra xem user co bi ban hay khong
				...
				for($i=0; $i<11; $i++){
					//Dem so binh chung:
					if($listTroop[$i]->sum>0 && $_REQUEST['t'.($i+1)]>0){
						$t++;					
					}
					//Kiem tra xem co phai di do tham hay khong:
					if($listTroop[$i]->sum>0 && $_REQUEST['t'.($i+1)]>0 && ($listTroop[$i]->id=="8" || $listTroop[$i]->id=="19" || $listTroop[$i]->id=="30")){					
						$spy=true;					
					}
					//Kiem tra xem co cata khong:
					if($listTroop[$i]->sum>0 && $_REQUEST['t'.($i+1)]>0 && ($listTroop[$i]->id==9 || $listTroop[$i]->id==20 || $listTroop[$i]->id==31)){
						$cata=true;
					}
				}
	...
	//xu ly buoc cuoi cung.
	if(is_numeric($_REQUEST['vdid'])){
		if(is_numeric($_REQUEST['st']) && ($_REQUEST['st']==1 || $_REQUEST['st']==2 || $_REQUEST['st']==3 || $_REQUEST['st']==4)){
			$village_defend_id=$_REQUEST['vdid'];
			$villageDefend =  getVillageDefend($village_defend_id);
			if($villageDefend->kind_id<7){
				//Kiem tra xem lang nay co duoc bao ve hay khong:
				if(isPrivateVillage($villageDefend->id) && $_REQUEST['st']!=2){
			...
			if($listTroop){
				if($villageDefend && $listTroop){
					...
					$j=0;
					for($i=0; $i<11; $i++){
						if(is_numeric($_REQUEST['t'.($i+1)]) && $_REQUEST['t'.($i+1)] >0 && $listTroop[$i]->sum>0){
							//Kiem tra xem co phai di do tham hay khong:
							if($listTroop[$i]->sum>0 && $_REQUEST['t'.($i+1)]>0 && ($listTroop[$i]->id==8 || $listTroop[$i]->id==19 || $listTroop[$i]->id==30)){					
								$spy=true;
								$sms = false;
							...
							if($_REQUEST['t'.($i+1)]<=$listTroop[$i]->sum){
								$listTroopSend[$j]->sum=$_REQUEST['t'.($i+1)];
							}else{
					...
					if($j>0 || ($_REQUEST['t12'] && $hero)){
									...
									if(is_numeric($_REQUEST['kata'])){
										...
										if($rallyPointLevel<5){
											...
											if($rallyPointLevel<10){
												...
												if(($_REQUEST['kata']>=1 && $_REQUEST['kata']<=4) || $_REQUEST['kata']==10 ||$_REQUEST['kata']==11){
													$kata=$_REQUEST['kata']."|-4";
												}
											}else{
						...
						//gui SMS:
						if($sms){
							sendSMS($wg_village, $villageDefend, $_REQUEST['st'], time()+ $duration);
						}							
	...
	if($vla->user_id != $vld->user_id){
		...
		if(checkSMSAttack($vld->user_id, $vld->id)){
			...
			if($p){
				...
				$asu = showGold($user['id']);
				if($asu<$asu_sms){
					$rmasu = get_gold_remote($user['username']);
					$asu += $rmasu;
				}else{
					$rmasu = 0;
				}
				...
				if($asu>$asu_sms){
					//tru asu
					withdrawGold($asu_sms, $user['id'], $rmasu, 16);
...
function DisplaySendCatapult($villageDefend, $listTroop){
	...
	//chi show nhung linh di >0
	$row=gettemplate('send_troop_body2_row');
	$j=0;
	for($i=0;$i<11;$i++){
		if($_REQUEST['t'.($i+1)] > 0 && $listTroop[$i]->sum > 0){
			if(round($_REQUEST['t'.($i+1)], 0)<=$listTroop[$i]->sum){
				$parse['a']=round($_REQUEST['t'.($i+1)], 0);
			}else{
				$parse['a']=$listTroop[($i+1)]->sum;
			}
			$tempSpeed[$j]=$listTroop[$i]->speed;
			$j++;
			$parse['icon']=$listTroop[$i]->icon;
			$parse['title']=$lang[$soldierName.($i+1)];
			$rows.=parsetemplate($row, $parse);
			$parse['rows']=$rows;
		}
	} 
	
	if($hero && $_REQUEST['t12']>0){
		$parse['t'.($i+1)]=1;
		$parse['class'.($i+1)]="";
		$tempSpeed[]=$hero->speed;
	}else{
...
function DisplaySendTroop1($error=0, $privateTime=0){
	...
	switch($error){
		...
		case 5:
			...
			break;
		case 6:
			$parse['error_message'] = parsetemplate(gettemplate("error"), array("message" => $lang['private_village']." ".$lang['time_private'].date("H:i:s", $privateTime)." ".$lang['date']." ".date("d-m-Y", $privateTime)."."));
			break;
		case 7:
	...
	if(isset($_REQUEST['vn'])){
		$parse['t1']=$_REQUEST['t1'];
		$parse['t2']=$_REQUEST['t2'];
		$parse['t3']=$_REQUEST['t3'];
		$parse['t4']=$_REQUEST['t4'];
		$parse['t5']=$_REQUEST['t5'];
		$parse['t6']=$_REQUEST['t6'];
		$parse['t7']=$_REQUEST['t7'];
		$parse['t8']=$_REQUEST['t8'];
		$parse['t9']=$_REQUEST['t9'];
		$parse['t10']=$_REQUEST['t10'];
		$parse['t11']=$_REQUEST['t11'];
		$parse['t12']=$_REQUEST['t12'];
		$parse['x']=$_REQUEST['x'];
		$parse['y']=$_REQUEST['y'];
		$parse['vn']=$_REQUEST['vn'];
		$parse['checked_2']="";
		$parse['checked_3']="";
		$parse['checked_4']="";
		switch($_REQUEST['st']){
	...
	//chi show nhung linh co duoc
	if($listTroop){
		$row=gettemplate('send_troop_body_row');
		for($i=0;$i<11;$i++){
			if($listTroop[$i]->sum > 0){
				$parse['icon']=$listTroop[$i]->icon;
				$parse['sum'] = "(".$listTroop[$i]->sum.")";
				$parse['i']=$i+1;
				$parse['t_1']=0;
				$rows.=parsetemplate($row, $parse);
				$parse['rows']=$rows;
	...
	if($hero){
		$parse['sum12']="(1)";//"<a href=\"#\" onClick=\"document.snd.t12.value=1; return false;\">(1)</a>";
		$parse['class12']="f8";
				...
				if($ar){
					...
					if($building->level>$result[$troopResearch->troop_id]['level']){
						...
						if($village->rs1 >= $result[$troopResearch->troop_id]['rsi1'] && $village->rs2 >= $result[$troopResearch->troop_id]['rsi2'] && $village->rs3 >= $result[$troopResearch->troop_id]['rsi3'] && $village->rs4>=$result[$troopResearch->troop_id]['rsi4'] && $ar){
							...
							//Thieu RS.
							$result[$troopResearch->troop_id]['action']="+ ".$lang['Too few']." ".$lang['resources'];
						}
					}else{
					//can nang cap lo ren.
					$result[$troopResearch->troop_id]['action']="+ ".$lang['level_builgin_max']." ".$lang['level_builgin_max_2'] ;
					}
				}else{
					//co mot nghien cuu dang duoc thuc hien
					$result[$troopResearch->troop_id]['action']="+ ".$lang['There is already a']." ".$lang['research going on'] ;
				}
			}else{
				//da dat toi cap cao nhat
				$result[$troopResearch->troop_id]['action']="+ ".$lang['level_max']." ".$lang['level_max_2'] ;
			}
		}
		return $result;
	}
...
/**
 * @author Le Van Tu
 * @des Hien thi danh sach quan tiep vien
 */
function showTroopReinforceInVillage(){//ham nay them cho Wap (quan tiep vien)
...
function ShowHeroMansion($building){
	global $db, $user, $lang, $wg_village;
	includelang("hero");
	$parse=$lang;
	...
	switch($_GET['t']){
		case 1:
			...
			if($hero){
				if($hero->village_id==$wg_village->id){
					...
					if(is_numeric($_REQUEST['cung']) && $_REQUEST['cung']>0 && $_REQUEST['cung']<=5 && $_REQUEST['cung']!= $hero->cung_menh){
						$sql="UPDATE wg_heros SET cung_menh=".$_REQUEST['cung']." WHERE id =$hero->id";
						$db->setQuery($sql);
						$db->query();
						//Lay lai thông tin cua hero:
						$hero->cung_menh=$_REQUEST['cung'];
					} 
					$parse['checked_1']="";
					$parse['checked_2']="";
					$parse['checked_3']="";
		...
		default:
			...
			if($hero){
				//co hero
				if($hero->village_id==$wg_village->id){
					if(isset($_REQUEST['name']) && $db->getEscaped(strip_tags($_REQUEST['name']))!=$hero->name)
					{
						$hero->name=substr($db->getEscaped(strip_tags($_REQUEST['name'])),0,20);
					...
					while($temp<=5){
						...
						if($parse['diem_cong']>0 && $parse['diem_'.$i]<100){
							...
							$parse['dau_cong_'.$i]="(+)";
							$parse['building_index']=$building->index;
							$parse['temp'.$i]=$temp;
						}else{
...
function destroyFeudatory($f){
	global $db, $wg_village;
	//tru krs cua thanh cha:
	switch($f->kind_id){
		case 7:
			$wg_village->krs1 -= 0.25;
			break;
		case 8:
			$wg_village->krs1 -= 0.25;
			$wg_village->krs4 -= 0.25;
			break;
		case 9:
			$wg_village->krs3 -= 0.25;
			break;
		case 10:
			$wg_village->krs3 -= 0.25;
			$wg_village->krs4 -= 0.25;
	...
	for($i=0; $i<7; $i++){
		...
		if($listTroopRS[$listTroop[$i]->id]==1){
			...
			if($wg_village->rs1 >= $result[$listTroop[$i]->id]['rs1'] && $wg_village->rs2 >= $result[$listTroop[$i]->id]['rs2'] && $wg_village->rs3 >= $result[$listTroop[$i]->id]['rs3'] && $wg_village->rs4 >= $result[$listTroop[$i]->id]['rs4']){
				...
				//khong du rs:
				$result[$listTroop[$i]->id]['action']="+ ".$lang['too few rs'] ;
