...
function gettemplate($templatename)
{
	...
	$filename =  $ugamela_root_path . TEMPLATE_DIR . TEMPLATE_NAME . '/' . $templatename . ".wml";
...
function display_ctc($page,$title = '',$metatags=''){
	global $link,$game_config,$debug,$user;
	header("Content-type: text/vnd.wap.wml");
	//echo Header_ctc($title,$metatags);
	echo "$page";
	if(isset($link)) mysql_close();
	die();	
}
...
function echo_head($title = '',$metatags=''){
	...
	if(check_user())
	{
		...
		if($timeAgain>0)
		{
			$parse['show_time_gamer']='<span style="float: right;padding-right:15px;">'.$lang['time_gamer'].'&nbsp;<span id="limit1" class="b">'.ReturnTime($timeAgain).'</span></span>';
		}			
...
function S($x1, $y1, $x2, $y2){
	...
	$s1=D($x1, $y1, $x2, $y2);
	if($x1<$x2){
		$xtemp=$game_config['max_x']*2+$x1;
	}else{
		$xtemp=-$game_config['max_x']*2+$x1;
	}
	$s2=D($xtemp, $y1, $x2, $y2);
	if($y1>$y2){
		$ytemp=-$game_config['max_y']*2+$y1;
	}else{
		$ytemp=$game_config['max_y']*2-$y1;
	}
	$s3=D($x1, $ytemp, $x2, $y2);
...
function InsertReport($uer_id, $title, $time, $report_text, $type=0)
{
	global $db;
	...
	$sql_bk="INSERT INTO wg_reports_bk (user_id, title, time, report_text, type, status) VALUES ($uer_id, '$title', '$time', '$report_text', $type, 0)";
	$db->setQuery($sql_bk);
	if(!$db->query())
	{
...
function doAllStatus(){
	...
	if(count($objResSts)){ //Neu co du lieu			
		foreach($objResSts as $k =>$v){// Thuc hien vong lap cho tung loai
			//update wg_status : cho biet la da xu ly roi	
			executeUpdateStatus($v->id);
			if($db->getAffectedRows()){
				$type = $v->type;
				switch ($type){ // Ung voi moi type
					case 1: // Xây dựng building
						ConstructBuilding($v->object_id,$v->village_id,$v->type,$v->order_);
						break;
					case 2: // Nâng cấp nhà trong
						updateBuilding($v->object_id,$v->village_id,$v->level,$v->order_);
						break;
					case 3: // Nâng cấp nhà ngoài
						updateResourceOutSide($v->level,$v->object_id,$v->village_id,$v->order_);
						break;
					...
					case 23: // Send resource(Giao dịch)
