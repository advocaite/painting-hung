...
function getListExchangeFire($alliance_id,$userid)
{
	...
	foreach ($wg_ally_members as $key=>$value)
	{
		if($value->user_id==$userid)
		{
			$array=NULL;
			$lis=NULL;
			$sql="SELECT id,title,type FROM wg_reports WHERE user_id=".$userid." AND ((type =2)0r(type =4)) ORDER BY time DESC LIMIT 5";
			$db->setQuery($sql);
			$wg_reports=NULL;
...
global $db,$wg_village_kinds,$wg_oasis_troop,$wg_village,$timeAgain,$user;
