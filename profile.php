...
function returnListMedal($wg_history_top10)
{
    global $lang;
    $parse = $lang;
    ...
    if ($wg_history_top10)
    {
        ...
        {
            $array[$v->week_nd]['value'] = $v->week_nd;
            $char                        = "'" . $lang['Medal'] . " " . $v->rank . "'";

            if ($v->type == 1) // xoa cai nay thi IE se loi show hinh anh va title
            {
                $array[$v->week_nd]['titile_attack'] = 'ONMOUSEOVER="ddrivetip(' . $char . ');"';
                $array[$v->week_nd]['Attack_medal']  = 'images/huanchuong/cong/' . $v->rank . '.png';
            }

            if ($v->type == 2)
            {
                $array[$v->week_nd]['titile_defend'] = 'ONMOUSEOVER="ddrivetip(' . $char . ');"';
                $array[$v->week_nd]['Defend_medal']  = 'images/huanchuong/thu/' . $v->rank . '.png';
            }

            if ($v->type == 3)
            {
                $array[$v->week_nd]['titile_resource'] = 'ONMOUSEOVER="ddrivetip(' . $char . ');"';
                $array[$v->week_nd]['Resource_medal']  = 'images/huanchuong/tainguyen/' . $v->rank . '.png';
            }
        }
        ...
        foreach ($array as $k)
        {
            $parse['num_week']        = $k['value'];
            $parse['titile_attack']   = $k['titile_attack'];
            $parse['titile_defend']   = $k['titile_defend'];
            $parse['titile_resource'] = $k['titile_resource'];
            $parse['Attack_medal']    = 'images/un/a/x.gif';

            if ($k['Attack_medal'] != '')
            {
                $parse['Attack_medal'] = $k['Attack_medal'];
            }
            ...
            if ($k['Defend_medal'] != '')
            {
                $parse['Defend_medal'] = $k['Defend_medal'];
            }
            ...
            if ($k['Resource_medal'] != '')
            {
                $parse['Resource_medal'] = $k['Resource_medal'];
            }

            $string .= parsetemplate(gettemplate('profile_medal_row'), $parse);
            ;
        }
    }
...
if (isset($_GET['uid']) && is_numeric($_GET['uid']))
{
    ...
    else
    {
        ...
        $parse['change_profile_or_write_message'] = '<a href="messages.php?tab=1&username=' . $userInfo->username . '">'
            . $lang['Send message'] . '</a>';
        $parse['phone']                           = '';

        if ($user['alliance_id'] > 0)
        {
            $sql = "SELECT privilege FROM wg_ally_members WHERE user_id=" . $user['id'] . " AND ally_id="
                . $user['alliance_id'];
            $db->setQuery($sql);
            ...
            if ($privilege)
            {
                ...
                if ($permission[7] == 1)
                {
                    $sql = "SELECT id FROM wg_ally_members WHERE user_id=" . $userId . " AND ally_id="
                        . $user['alliance_id'];
                    $db->setQuery($sql);
                    ...
                    if (!$wg_ally_members)
                    {
                        $parse['invite_ally'] = '<a href="allianz.php?event=6&username=' . $userInfo->username . '">'
                            . $lang['Invite_ally'] . '</a>';
                    }
                }
            }
...
if ($wg_history_top10)
{
    $parse['row_medal']     = returnListMedal($wg_history_top10);
...
switch ($task)
{
    case "1": //edit Profile
        ...
        foreach ($city as $city_row)
        {
            if ($userInfo->birthplace == $city_row['id'])
            {
                $parse['option_city'] .= "<option value='" . $city_row['id'] . "' selected>" . $city_row['name']
                    . "</option>";
            }
            else
            {
                $parse['option_city'] .= "<option value='" . $city_row['id'] . "'>" . $city_row['name'] . "</option>";
            }
        }
        ...
        if ($_GET['act'] == 1)
        {
            ...
            if ($isCheck)
            {
                ...
                if (empty($check))
                {
                    ...
                    $parse['message'] = $lang['error input char'];
                }
                else
                {
