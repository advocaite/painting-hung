...
function returnRowTop10($name,$userid,$week_nd)
{
	global $db;
...
elseif($_GET['tab']==4)
{
	...
	$sql="SELECT SUM( population ) AS workers, res1. * , 
				...
				SELECT data1.id, data1.name,data1.user_id,data1.rank as level,
				(SELECT username FROM wg_users WHERE id=data1.user_id) AS MC, data1.point, data2. * 
				...
				WHERE t2.right_=1
				GROUP BY res1.id
				ORDER BY level DESC ,res1.member DESC , medium_mem DESC , point DESC";
	if($sum_allies>0)
	{	
		$tpl='ranking_allies';	
		...
		foreach ($list_allies as $key =>$ptu)
		{
			...
			$parse['name_boss']=$ptu->MC;
			$parse['uid']=$ptu->user_id;
			$parse['worker_tb']=$ptu->medium_mem;
			$parse['name_allie']=$ptu->name;
			$parse['level'] = $ptu->level;
			$count++;			
...
elseif($_GET['tab']==6)
{
	$tpl='ranking_top10';
	$week_nd=date("W");
	$sql="SELECT attack_point,defend_point,resource FROM wg_top10 WHERE  user_id=".$user['id']." 
	AND week_nd=".$week_nd." LIMIT 1";
	$db->setQuery($sql);
	$wg_top10_user=NULL;
	$db->loadObject($wg_top10_user);
	$string1=returnRowTop10('attack_point',$user['id'],$week_nd);	
	$parse['row_attack']=$string1[1];
	$parse['attack_user']='';	
	if($string1[0]==0)
	{		
		...
		$parse['uid']=$user['id'];
		$parse['point']=$wg_top10_user->attack_point;
		$parse['attack_user']=parsetemplate(gettemplate('ranking_top10_user'),$parse);
	}
	$string2=returnRowTop10('defend_point',$user['id'],$week_nd);
	$parse['row_defend']=$string2[1];	
	$parse['defend_user']='';	
	if($string2[0]==0)
	{		
		...
		$parse['username']=$user['username'];
		$parse['uid']=$user['id'];
		$parse['point']=$wg_top10_user->defend_point;
		$parse['defend_user']=parsetemplate(gettemplate('ranking_top10_user'),$parse);
	}
	$string3=returnRowTop10('resource',$user['id'],$week_nd);
	$parse['row_resource']=$string3[1];
	$parse['resource_user']='';
	if($string3[0]==0)
	{		
		...
		$parse['username']=$user['username'];
		$parse['uid']=$user['id'];
		$parse['point']=$wg_top10_user->resource;
		$parse['resource_user']=parsetemplate(gettemplate('ranking_top10_user'),$parse);
	}
}
/*elseif($_GET['tab'] == 7)
{
	...
	FROM `wg_heros` AS tb INNER JOIN wg_users ON tb.user_id=wg_users.id WHERE `status` =1 ORDER BY `kinh_nghiem` DESC";
	$db->setQuery($sql);
	$wg_heros=NULL;
	...
	if($wg_heros)
	{
		if(count($_POST)<>0)
		{
			...
			{
				header("Location:ranking.php?tab=7");
				exit();
			}		
		...
		$a="'ranking.php?tab=7&page='+this.options[this.selectedIndex].value";
		$b="'_top'";
		$string='onchange="javascript:window.open('.$a.','.$b.')"';
		$parse['page']=''.$lang['40'].' <select name="page" class="fm" style="width:40px;height=40px;" '.$string.'>';
