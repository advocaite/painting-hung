...
function SearchKey()
{
	...
	if(keyword!='')
	{
		...
		{
			document.location.href="status.php?s=sbk&keyword="+keyword+"&Time=ToDay";			
		}
		else if(document.getElementById("day").value >0 && document.getElementById("month").value >0 && document.getElementById("year").value >0)
		{
			document.location.href="status.php?s=sbk&keyword="+keyword+"&SearchTime="+document.getElementById("day").value+"-"+document.getElementById("month").value+"-"+document.getElementById("year").value;
		}
		else
		{
			document.location.href="status.php?s=sbk&keyword="+keyword;			
		}
	}
	else
	{
		if(document.frm_status_list.today.checked)
		{
			document.location.href="status.php?s=sbk&Time=ToDay";			
		}
		else if(document.getElementById("day").value>0)
		{
			document.location.href="status.php?s=sbk&SearchTime="+document.getElementById("day").value+"-"+document.getElementById("month").value+"-"+document.getElementById("year").value;
		}
		else
		{
			document.location.href="status.php?s=sbk";
		}
	}
	return false;
}
...
<div id="column-content">
  <div id="content">
   <p class="title_menu"><a href="status.php">{Wg Status}</a> | <span style="background:#0099FF"><a href="status.php?s=sbk">{Wg Status backup}</a></span></p>
    <form name="frm_status_list" method="POST">
      ...
      <!--<table width="100%">
	  <tr>
          <td align="center"><input type="button" name="find" value="Xóa {total_record} record" class="fm" onclick="javascript:window.open('status.php?s=','_top');"/></td>
        </tr>
      </table>-->
      <table width="100%">
        <tr>
