...
function Update(id)
{
	...
	if(value!='')
	{
		window.open('view_error.php?tab=3&fix_name='+value+'&id='+id,'_top');
	}
}
function SearchKey()
{
	...
	if(keyword !='')
	{
		document.location.href="view_error.php?tab=3&keyword="+keyword;	
		return false;		
	}
	document.location.href="view_error.php?tab=3";
	return false;				
}
</script>
<div id="column-content">
  <div id="content">
    <p><a href="view_error.php?tab=1">{Name Hero}</a> | <a href="view_error.php?tab=2">{Name Allies}</a> | <span style="background:#0099FF"><a href="view_error.php?tab=3">{Name Village}</a></span></p>
    <form name="frm_same_pc" method="post" onsubmit="return SearchKey();">
	<fieldset>
      <table align="center" width="100%">
        <tr>
          <td colspan="7" align="center"><b style="font-size:18px; color:#FF0000">{List Village}</b><br />{Search by name} : <input type="Text" id="txt_name" name="txt_name" value="{value_name}" size="20" maxlength="20" class="fm fm110">&nbsp;&nbsp;<input type="submit" name="submit_player" class="fm" value="{Search}"/></td>
        </tr>       
      </table>
	  </fieldset>	   
