...
if(!empty($_GET['a']) && is_numeric($_GET['a']) && isset($_SESSION['UpdateBuilding'.$_GET['a']]) )
{
	...
	{	
		...
		if($count==0 && isset($_SESSION['UpdateBuilding'.$get_a]))
		{
			...
			{
				$wg_village->rs1=$Lumber1 - $array[4];
				$wg_village->rs2=$Clay1 - $array[5];
				$wg_village->rs3=$Iron1 - $array[6];
				$wg_village->rs4=$Crop1 - $array[7];
				...
				if(!$db->query())
				{
					$wg_village->rs1=$Lumber1;
					$wg_village->rs2=$Clay1;
					$wg_village->rs3=$Iron1;
					$wg_village->rs4=$Crop1;
					globalError2($sql);
				}					
				for($i=1;$i<38;$i++)
				{
