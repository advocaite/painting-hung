...
function getListExchangeFire($alliance_id,$userid)
{
	...
	foreach ($wg_ally_members as $key=>$value)
	{
		if($value->user_id==$userid)
		{
			$array=NULL;
			$lis=NULL;
			$sql="SELECT id,title,type FROM wg_reports WHERE user_id=".$userid." AND type IN(2,4) ORDER BY time DESC LIMIT 5";
			$db->setQuery($sql);
			$wg_reports=NULL;
...
global $db,$wg_village_kinds,$wg_oasis_troop,$wg_village,$timeAgain,$user,$game_config;
...
{
	$x=$_GET['a'];$y=$_GET['b'];
	if((abs($x)>$game_config['max_x'])||(abs($y)>$game_config['max_y'])) header("Location:map.php");
	...
	if($query)
	{
		...
		if($query->user_id >0)
		{
			$sql="SELECT tb1.name,tb1.nation_id,tb1.workers,tb1.child_id,tb3.username,tb3.villages_id 
			FROM wg_villages AS tb1,wg_users AS tb3 WHERE tb1.id=".$query->id." AND tb3.id=".$query->user_id."";
			$db->setQuery($sql);
			$wg_villages=null;
			...
			$parse['capital']='';
			if($query->id == $wg_villages->villages_id)
			{
				$parse['capital']='[ '.$lang['Capital'].' ]';
			}			
			...
			$name_tribe=array(1=>'Arabia',2=>'Mongo',3=>'Sunda');
			$parse['name_tribe']=$name_tribe[$wg_villages->nation_id];
			$parse['display']='none';
		...
		{	
			...
			else
			{
				...
				else
				{
					...
					$parse['images']='images/warning1.gif';
					$parse['nametroop']='';
					$list=parsetemplate(gettemplate('row_troop_oasis'), $parse);	
				}							
